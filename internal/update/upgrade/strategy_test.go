...
func TestRunStrategy_BetaShevanioAISelfUpgradeUsesGoInstallMain(t *testing.T) {
	origExecCommand := execCommand
	t.Cleanup(func() { execCommand = origExecCommand })
	...
	_, err := runStrategy(context.Background(), r, profile)
	if err != nil {
		t.Fatalf("runStrategy beta shevanio-ai: unexpected error: %v", err)
	}

	if gotName != "go" {
		t.Fatalf("exec name = %q, want %q", gotName, "go")
	}
	wantArgs := []string{"install", "github.com/shevanio/shevanio-ai/v2/cmd/shevanio-ai@main"}
	if len(gotArgs) != len(wantArgs) || gotArgs[0] != wantArgs[0] || gotArgs[1] != wantArgs[1] {
		t.Fatalf("exec args = %v, want %v", gotArgs, wantArgs)
	}
	for _, want := range []string{
		"GONOSUMDB=github.com/shevanio/shevanio-ai/v2",
		"GOPRIVATE=github.com/shevanio/shevanio-ai/v2",
		"GONOPROXY=github.com/shevanio/shevanio-ai/v2",
	} {
...
func TestGoProxyBypassEnvPreservesExistingPatterns(t *testing.T) {
	module := "github.com/shevanio/shevanio-ai/v2"
	...
	for _, want := range []string{
		"PATH=/usr/bin",
		"GONOSUMDB=github.com/shevanio/shevanio-ai/v2,example.com/private",
		"GOPRIVATE=github.com/shevanio/shevanio-ai/v2,github.com/acme/*",
		"GONOPROXY=github.com/shevanio/shevanio-ai/v2",
	} {
...
// TestEffectiveMethodShevanioAIOnWindowsUsesFailClosedBinaryPolicy verifies that
// Windows never routes shevanio-ai through a remote installer, and that when no
...
func TestEffectiveMethodShevanioAIOnWindowsUsesFailClosedBinaryPolicy(t *testing.T) {
	tests := []struct {
		name string
		tool update.ToolInfo
	}{
		...
		},
		{
			...
			tool: update.ToolInfo{Name: "shevanio-ai", InstallMethod: update.InstallScript},
		},
	}
...
// --- TestEffectiveMethod_NonShevanioAIToolsOnWindowsUseBinary ---

// TestEffectiveMethod_NonShevanioAIToolsOnWindowsUseBinary verifies that tools
// OTHER than shevanio-ai on Windows still use their declared install method
// (binary, script, etc.).
func TestEffectiveMethod_NonShevanioAIToolsOnWindowsUseBinary(t *testing.T) {
	tests := []struct {
		name string
		tool update.ToolInfo
...
func TestHomebrewPackageInstalledWithRequiresActiveBrewPath(t *testing.T) {
	...
	if homebrewPackageInstalledWith(func(string, ...string) *exec.Cmd { return mockCmd("true") }, func(string) (string, error) { return "", errors.New("not found") }, "shevanio-ai") {
...
func TestRunStrategyOpenCodePluginUpgradesMaterializedPackage(t *testing.T) {
	...
	execCommand = func(name string, args ...string) *exec.Cmd {
		...
		cmd.Env = append(os.Environ(),
			"SHEVANIO_AI_UPGRADE_HELPER=1",
			"SHEVANIO_AI_UPGRADE_HELPER_CWD_FILE="+cwdFile,
			"SHEVANIO_AI_UPGRADE_HELPER_MANIFEST_PATH="+filepath.Join(pkgDir, "package.json"),
			"SHEVANIO_AI_UPGRADE_HELPER_MANIFEST_VERSION=0.2.0",
		)
		return cmd
	}
...
func TestOpenCodePluginUpgradeHelperProcess(t *testing.T) {
	if os.Getenv("SHEVANIO_AI_UPGRADE_HELPER") != "1" {
		return
	}
	cwd, err := os.Getwd()
	if err != nil {
	...
	if err := os.WriteFile(os.Getenv("SHEVANIO_AI_UPGRADE_HELPER_CWD_FILE"), []byte(cwd), 0o644); err != nil {
		_, _ = os.Stderr.WriteString(err.Error())
		os.Exit(2)
	}
	if manifestPath := os.Getenv("SHEVANIO_AI_UPGRADE_HELPER_MANIFEST_PATH"); manifestPath != "" {
		version := os.Getenv("SHEVANIO_AI_UPGRADE_HELPER_MANIFEST_VERSION")
...
func TestBrewUpgrade_RunsUpdateBeforeUpgrade(t *testing.T) {
	...
	err := brewUpgrade(context.Background(), update.UpdateResult{Tool: update.ToolInfo{Name: "shevanio-ai"}}, update.HomebrewFormula)
	if err != nil {
...
func TestBrewUpgrade_UpdateFailureIsNonFatal(t *testing.T) {
	...
	execCommand = func(name string, args ...string) *exec.Cmd {
		...
		return mockCmd("echo", "Upgraded shevanio-ai")
	}
...
func TestBrewUpgrade_FormulaToolUsesFormulaTrust(t *testing.T) {
	...
	if err := brewUpgrade(context.Background(), update.UpdateResult{Tool: update.ToolInfo{Name: "shevanio-ai"}}, update.HomebrewFormula); err != nil {
		t.Fatalf("brewUpgrade: unexpected error: %v", err)
	}

	if len(trustArgs) != 2 || trustArgs[0] != "--formula" || trustArgs[1] != "shevanio/tap/shevanio-ai" {
		t.Fatalf("brew trust args = %v, want [--formula shevanio/tap/shevanio-ai]", trustArgs)
	}
}

func TestHomebrewFailureAdviceTapTrust(t *testing.T) {
	output := `Error: Refusing to load formula shevanio/tap/shevanio-ai from untrusted tap.
Run brew trust --formula shevanio/tap/shevanio-ai to trust it.`
	advice := homebrewFailureAdvice("shevanio-ai", output)
	for _, want := range []string{
		"brew trust --formula shevanio/tap/shevanio-ai",
		"brew upgrade --formula shevanio-ai",
	} {
...
Homebrew's Linux sandbox requires rootless Bubblewrap and unprivileged user namespaces.`
	...
	for _, want := range []string{
		...
		"HOMEBREW_NO_SANDBOX_LINUX=1 brew upgrade --formula shevanio-ai",
	} {
		if !strings.Contains(advice, want) {
...
// SHEVANIO_AI_CHANNEL is unset or "stable", engramBinaryUpgrade delegates to
...
func TestEngramBinaryUpgrade_StableChannelCallsDownloadFn(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Setenv("SHEVANIO_AI_CHANNEL", tt.envVal)
...
// SHEVANIO_AI_CHANNEL=beta, engramBinaryUpgrade delegates to
