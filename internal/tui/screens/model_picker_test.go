...
func TestRenderModelPickerScrollsToReviewAgents(t *testing.T) {
	...
	if strings.Contains(output, model.CanonicalManagedIdentity.Display) {
...
func TestModelPickerRows_OrchestratorIsFirst(t *testing.T) {
	rows := ModelPickerRows()
	if rows[0] != model.CanonicalManagedIdentity.Display {
		t.Fatalf("ModelPickerRows()[0] = %q, want %q", rows[0], model.CanonicalManagedIdentity.Display)
	}
	identity, ok := ModelPickerRowAt(ModelPickerState{}, 0)
	if !ok || identity.AgentID != model.CanonicalManagedIdentity.Actor {
		t.Fatalf("orchestrator row identity = %#v, want actor %q", identity, model.CanonicalManagedIdentity.Actor)
	}
}
...
func TestHandleModelNav_OrchestratorRowAssignsOnlyOrchestrator(t *testing.T) {
	state := makeTestState(0) // row 0 = Shevanio orchestrator
	...
	// The canonical managed orchestrator key must be set.
...
func TestHandleModelNav_SubAgentRow_AssignsCorrectPhase(t *testing.T) {
	...
	for i, expectedPhase := range phases {
		t.Run(expectedPhase, func(t *testing.T) {
			...
			if _, exists := updated[SDDOrchestratorPhase]; exists {
				t.Errorf("Shevanio orchestrator should not be assigned; assignments: %v", updated)
			}
		})
	}
}

// ─── SDDOrchestratorPhase identity ────────────────────────────────────────

func TestSDDOrchestratorPhaseCanonicalIdentity(t *testing.T) {
	if SDDOrchestratorPhase != model.CanonicalManagedIdentity.Actor {
		t.Fatalf("SDDOrchestratorPhase = %q, want %q", SDDOrchestratorPhase, model.CanonicalManagedIdentity.Actor)
	}
}
