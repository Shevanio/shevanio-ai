...
func UninstallModeOptions() []UninstallModeOption {
	return []UninstallModeOption{
		...
		},
		{
			...
			Description: "Remove all shevanio-ai managed configuration from all agents",
		},
		{
			Mode:        model.UninstallModeFullRemove,
			Label:       "Full Uninstall & Remove Binary",
			Description: "Remove all configuration AND delete the shevanio-ai binary itself",
		},
		{
...
func RenderUninstallMode(cursor int) string {
	...
	b.WriteString("\n\n")
	b.WriteString(styles.SubtextStyle.Render("Choose how you want to uninstall shevanio-ai:"))
	b.WriteString("\n\n")
...
func RenderUninstall(selected []model.AgentID, cursor int) string {
	...
	b.WriteString("\n\n")
	b.WriteString(styles.SubtextStyle.Render("Select the agents whose shevanio-ai managed configuration should be removed."))
	b.WriteString("\n\n")
...
func RenderUninstallComponents(selected []model.ComponentID, cursor int) string {
	...
	b.WriteString(styles.HelpStyle.Render("Use j/k to move, space to toggle, enter to continue."))
	b.WriteString("\n\n")
	b.WriteString(styles.SubtextStyle.Render("Select which shevanio-ai managed components should be removed from the selected agents."))
	b.WriteString("\n\n")
...
func RenderUninstallConfirm(mode model.UninstallMode, selected []model.AgentID, components []model.ComponentID, profilesToRemove []string, engramScope model.EngramUninstallScope, engramProjectScopeAvailable bool, cursor int, operationRunning bool, spinnerFrame int) string {
	...
	case model.UninstallModeFull:
		...
		b.WriteString("\n\n")
		b.WriteString(styles.UnselectedStyle.Render("This will remove all shevanio-ai managed configuration from all supported agents."))
		b.WriteString("\n")
	case model.UninstallModeFullRemove:
		b.WriteString(styles.ErrorStyle.Render("Mode: Full Uninstall & Remove Binary"))
		b.WriteString("\n\n")
		b.WriteString(styles.UnselectedStyle.Render("This will remove all shevanio-ai managed configuration from all agents"))
		b.WriteString("\n")
		b.WriteString(styles.ErrorStyle.Render("AND delete the shevanio-ai binary itself."))
		b.WriteString("\n\n")
	...
	if err != nil {
		...
		if mode == model.UninstallModeCleanInstall {
			...
			if syncErr != nil {
				...
				b.WriteString("\n\n")
				b.WriteString(styles.WarningStyle.Render("You can run 'shevanio-ai sync' manually to retry."))
			} else {
