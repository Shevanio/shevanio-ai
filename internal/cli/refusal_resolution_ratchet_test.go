...
// either
//
//  (a) names a runnable continuation (a `shevanio-ai ...` invocation in the
//      message),
...
//
//        SHEVANIO_AI_REFUSAL_RATCHET_UPDATE=1 go test ./internal/cli \
...
// continuation. Requiring a lowercase letter directly after "shevanio-ai "
// excludes prose that mentions the product name without naming a command.
var refusalRatchetNamedContinuationRegexp = regexp.MustCompile(`shevanio-ai [a-z][a-z-]*`)
...
func TestRefusalRatchetClassifiesSyntheticSites(t *testing.T) {
	...
	t.Run("naming a shevanio-ai continuation satisfies", func(t *testing.T) {
		analysis := refusalRatchetMustAnalyze(t, header+
			"func f(l string) error {\n\treturn fmt.Errorf(\"blocked: run `shevanio-ai review reopen-results --lineage %s`\", l)\n}\n")
	...
	t.Run("marker plus named continuation is contradictory", func(t *testing.T) {
		analysis := refusalRatchetMustAnalyze(t, header+
			...
			"\treturn errors.New(\"blocked: run shevanio-ai review finalize\")\n"+
			"}\n")
	...
	t.Run("concatenated literals are analyzed as one message", func(t *testing.T) {
		analysis := refusalRatchetMustAnalyze(t, header+
			"func f() error {\n\treturn errors.New(\"blocked: run \" + \"shevanio-ai review status\")\n}\n")
...
func TestEveryProductionRefusalNamesResolutionOrDeclaresByDesign(t *testing.T) {
	...
	if os.Getenv("SHEVANIO_AI_REFUSAL_RATCHET_UPDATE") == "1" {
	...
	if err != nil {
		t.Fatalf("missing baseline %s -- run: SHEVANIO_AI_REFUSAL_RATCHET_UPDATE=1 go test ./internal/cli -run TestEveryProductionRefusalNamesResolutionOrDeclaresByDesign -count=1 (%v)", baselinePath, err)
	}
	...
	for _, key := range newKeys {
		...
		t.Errorf("NEW refusal with no named resolution: %s:%d %s(%q)\n"+
			"  Either name the runnable continuation in the message (`shevanio-ai ...`),\n"+
	...
	if removed > 0 {
		...
		t.Logf("      Tighten the baseline with: SHEVANIO_AI_REFUSAL_RATCHET_UPDATE=1 go test ./internal/cli -run TestEveryProductionRefusalNamesResolutionOrDeclaresByDesign -count=1")
	}
}
...
func refusalRatchetAnalyzeSource(fileLabel, source string) (refusalRatchetAnalysis, error) {
	...
	ast.Inspect(file, func(node ast.Node) bool {
		...
		case marker != nil && named:
			analysis.problems = append(analysis.problems, fmt.Sprintf(
				"%s:%d is contradictory: the message names a `shevanio-ai` continuation AND the site declares by-design %q; a refusal either has a runnable exit or it does not -- the two claims are mutually exclusive",
				fileLabel, line, marker.shape))
		case marker != nil:
