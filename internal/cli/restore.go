...
// RunRestore is the top-level entry point for `shevanio-ai restore [args]`.
...
func runRestoreWithHomeDir(args []string, restorer RestoreFunc, stdout io.Writer, stdin io.Reader, homeDir string) error {
	...
	if len(positional) == 0 {
		return fmt.Errorf("usage: shevanio-ai restore [--list | latest | <id>] [--yes]")
	}
...
// and the shevanio-ai version that created the backup when known.
...
func resolveRestoreTarget(target string, backups []backup.Manifest) (backup.Manifest, error) {
	...
	return backup.Manifest{}, fmt.Errorf("backup %q not found — use `shevanio-ai restore --list` to see available backups", target)
}
...
func backupRootDir(homeDir string) string {
	return homeDir + "/.shevanio-ai/backups"
}
