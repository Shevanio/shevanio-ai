...
const reviewVerificationEvidenceSchemaID = "https://shevanio-ai.dev/schema/review/verification-evidence/v1"
...
var reviewVerificationEvidenceSchema = fmt.Sprintf(
	`{"$schema":"https://json-schema.org/draft/2020-12/schema","$id":"`+reviewVerificationEvidenceSchemaID+`","title":"Shevanio AI captured final verification evidence","description":"Raw final test or verification evidence content captured by review capture-evidence. It is not a structured JSON document: any non-empty content up to the native artifact bound is accepted.","type":"string","minLength":1,"maxLength":%d}`,
	reviewResultArtifactLimit,
)

const reviewVerificationEvidenceRecordSchema = `{"$schema":"https://json-schema.org/draft/2020-12/schema","$id":"https://shevanio-ai.dev/contracts/review-integration/v1/schemas/verification-evidence.schema.json","title":"Shevanio AI outcome-bearing verification evidence record","type":"object","additionalProperties":false,"required":["schema","version","lineage_id","authority_revision","target_identity","candidate_tree","paths_digest","paths","ledger_ids","raw_payload_sha256","raw_payload_bytes","outcome","record_digest"],"properties":{"schema":{"const":"shevanio-ai.review-verification-evidence/v2"},"version":{"const":2},"lineage_id":{"type":"string","maxLength":128,"pattern":"^[a-z0-9]+(?:-[a-z0-9]+)*$"},"authority_revision":{"$ref":"#/$defs/sha256"},"target_identity":{"$ref":"#/$defs/sha256"},"candidate_tree":{"type":"string","pattern":"^(?:[0-9a-f]{40}|[0-9a-f]{64})$"},"paths_digest":{"$ref":"#/$defs/sha256"},"paths":{"type":"array","uniqueItems":true,"items":{"type":"string","minLength":1}},"ledger_ids":{"type":"array","uniqueItems":true,"items":{"type":"string","minLength":1}},"raw_payload_sha256":{"$ref":"#/$defs/sha256"},"raw_payload_bytes":{"type":"integer","minimum":1,"maximum":4194304},"outcome":{"enum":["passed","verification_failed","procedural_tooling_failed"]},"record_digest":{"$ref":"#/$defs/sha256"}},"$defs":{"sha256":{"type":"string","pattern":"^sha256:[0-9a-f]{64}$"}}}`

const reviewResultDryRunResponseSchema = `{"$schema":"https://json-schema.org/draft/2020-12/schema","$id":"https://shevanio-ai.dev/contracts/review-integration/v2/schemas/capture-result-dry-run.schema.json","title":"Shevanio AI capture-result admission dry run","type":"object","additionalProperties":false,"required":["schema","operation","validation","lineage_id","lens","selected_order","subject_hash"],"properties":{"schema":{"const":"shevanio-ai.review-capture-result-dry-run/v1"},"operation":{"const":"review/capture-result"},"validation":{"const":"accepted"},"lineage_id":{"type":"string","pattern":"^[a-z0-9]+(?:-[a-z0-9]+)*$"},"lens":{"enum":["risk","resilience","readability","reliability","review-risk","review-resilience","review-readability","review-reliability"]},"selected_order":{"type":"integer","minimum":0,"maximum":3},"subject_hash":{"$ref":"#/$defs/sha256"},"admission_decision":{"const":"completed"}},"$defs":{"sha256":{"type":"string","pattern":"^sha256:[0-9a-f]{64}$"}}}`
