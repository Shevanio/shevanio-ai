...
func runReviewInspectCandidate(args []string, help io.Writer, deps reviewInspectCandidateDeps) ([]byte, error) {
	...
	if *purpose != reviewTargetedValidationPurpose {
		...
		if flags.NArg() != 0 || strings.TrimSpace(*repositoryContext) == "" || strings.TrimSpace(*revision) == "" ||
			...
			return nil, reviewPreflightError(errors.New("review inspect-candidate requires the exact provider-issued repository context, revision, lineage, target, lens, and order; run `shevanio-ai review inspect-candidate --help` for the closed command forms"))
		}
	}
	...
	case "name-status", "numstat":
		if pathProvided || sideProvided {
			return nil, reviewPreflightError(errors.New("global candidate inspection does not accept a path index or side; run `shevanio-ai review inspect-candidate --help` for the closed command forms"))
		}
	case "stat", "patch":
		if !pathProvided || *pathIndex < 0 || sideProvided {
			return nil, reviewPreflightError(errors.New("path candidate inspection requires only a non-negative --path-index; run `shevanio-ai review inspect-candidate --help` for the closed command forms"))
		}
	case "object":
		if !pathProvided || *pathIndex < 0 || !sideProvided || (*side != "base" && *side != "candidate") {
			return nil, reviewPreflightError(errors.New("object candidate inspection requires --path-index and --side base|candidate; run `shevanio-ai review inspect-candidate --help` for the closed command forms"))
		}
	default:
		return nil, reviewPreflightError(fmt.Errorf("unknown candidate inspection operation %q; run `shevanio-ai review inspect-candidate --help` for the closed command forms", *operation))
	}
	...
	if *purpose == reviewTargetedValidationPurpose {
		if flags.NArg() != 0 || strings.TrimSpace(*repositoryContext) == "" || strings.TrimSpace(*revision) == "" ||
			...
			return nil, reviewPreflightError(errors.New("review inspect-candidate targeted validation requires the exact provider-issued repository context, revision, lineage, target, and request hash; run `shevanio-ai review inspect-candidate --help` for the closed command forms"))
		}
		...
		if state.State != reviewtransaction.StateReviewing || state.InitialSnapshot.Identity != *target || record.Revision != *revision ||
			*order >= len(state.SelectedLenses) || state.SelectedLenses[*order] != *lens {
			return nil, reviewPreflightError(errors.New("candidate inspection binding does not match the current reviewing authority; refresh the exact native next_transition before retrying `shevanio-ai review inspect-candidate`"))
		}
