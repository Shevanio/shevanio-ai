...
// works" contract.
//
// WHAT IT PROVES: every `shevanio-ai review <verb>` continuation the product
...
// was named for. A structurally perfect `shevanio-ai review recover
...
// "review " excludes the usage banner's `shevanio-ai review <capabilities|...>`
// placeholder, which names no single verb.
var reviewNamedContinuationInvocationRegexp = regexp.MustCompile(`shevanio-ai review ([a-z][a-z-]*)`)
...
// refusals of its own), enumerates every named `shevanio-ai review <verb>`
...
func TestEveryNamedReviewContinuationIsStructurallyReal(t *testing.T) {
	...
	for _, site := range sites {
		if !verbs[site.verb] {
			t.Errorf("%s:%d names the continuation `shevanio-ai review %s`, but no such verb is dispatched (%q)",
		...
		for _, name := range site.flags {
			namedFlags++
			if !flags[name] {
				t.Errorf("%s:%d names `shevanio-ai review %s --%s`, but that verb's FlagSet defines no --%s (%q)",
...
// named `shevanio-ai review <verb>` continuation found in a string literal,
...
// reviewDispatchableReviewVerbs returns every verb `shevanio-ai review <verb>`
...
func reviewNamedContinuationVerbFlags(t *testing.T, verb string) map[string]bool {
	...
	if err != nil {
		t.Fatalf("`shevanio-ai review %s --help` failed: %v\n%s", verb, err, usage.String())
	}
	...
	if len(flags) <= 2 {
		t.Fatalf("`shevanio-ai review %s --help` listed no flags; the usage parsing is stale:\n%s", verb, usage.String())
	}
	return flags
}
