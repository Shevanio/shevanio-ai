...
// has to interpret: it is `shevanio-ai review capture-result`, and the input's
...
func TestNativeCaptureCollectArgumentsCarryRunnableTokens(t *testing.T) {
	...
	for _, argument := range input.Arguments {
		...
		if _, real := accepted[argument.Name]; !real {
			t.Errorf("collect argument %q is not a flag `shevanio-ai review capture-result` accepts: %v", argument.Name, sortedFlagNames(accepted))
		}
	}
}
...
func TestCollectCaptureOperationKindsAreClassifiedFromSource(t *testing.T) {
	...
	for _, operation := range operations {
		transition := reviewCollectTransition("capture_operation_kind_probe", ReviewTransitionInput{
			Name: "probe", Schema: "shevanio-ai.review-capture-operation-probe/v1", CaptureOperation: operation,
		...
		case strings.HasPrefix(operation, "review."):
			...
			if len(reviewVerbFlagNames(t, verb)) == 0 {
				t.Errorf("native capture operation %q names no runnable `shevanio-ai review %s` command", operation, verb)
			}
...
// reviewVerbFlagNames returns the flag names one `shevanio-ai review <verb>`
