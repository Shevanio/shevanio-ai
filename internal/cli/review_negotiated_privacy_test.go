...
func TestNegotiatedPrivacyGuardStillRejectsProviderPrivateFields(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			payload: `{"next_transition":{"kind":"collect","collect":{"inputs":[{"arguments":[{"name":"lineage","token":"--lineage=review-abc","store_path":"/home/u/.shevanio-ai/review"}]}]}}}`,
		...
		},
		{
			...
			payload: `{"next_transition":{"kind":"execute","execute":{"arguments":[{"name":"lineage","token":"/home/u/.shevanio-ai/review/authority.json"}]}}}`,
			want:    "next_transition.execute.arguments[].token",
		},
		{
			name:    "lock handle anywhere is rejected",
			payload: `{"next_transition":{"kind":"execute","execute":{"binding":{"lock":"/run/shevanio-ai.lock"}}}}`,
...
func TestNegotiatedPrivacyGuardMatchesTheNameCheckWhenNothingIsPublished(t *testing.T) {
	...
	payloads := []string{
		...
		`{"authority":{"store_path":"/home/u/.shevanio-ai"}}`,
