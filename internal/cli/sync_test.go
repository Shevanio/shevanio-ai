...
// finding 3: `shevanio-ai sync --agent cluade` (a typo) previously printed
...
func TestParseSyncFlagsMistypedFlagNamesTheSupportedFlags(t *testing.T) {
	...
	if !strings.Contains(msg, "shevanio-ai sync --help") {
		t.Fatalf("error = %q, want it to point at `shevanio-ai sync --help`", msg)
	}
...
// TestParseSyncFlagsHelpFlagRendersUsage proves `shevanio-ai sync --help` now
...
// surface audit finding 5: `shevanio-ai sync claude` (a positional agent name
...
// Version, so every `shevanio-ai sync` silently re-inflated the slim section
...
func TestComponentSyncStepRunsPersonaInjectForSync(t *testing.T) {
	...
	if !strings.Contains(string(body), "<!-- shevanio-ai:persona -->") {
...
func TestComponentSyncStepWritesPiPersonaToWorkspaceAndReportsChangedFile(t *testing.T) {
	home := t.TempDir()
	workspace := t.TempDir()
	path := filepath.Join(workspace, ".pi", "shevanio-ai", "persona.json")
	var changed []string
	step := componentSyncStep{
		id:           "sync:persona",
		component:    model.ComponentPersona,
	...
	if _, err := os.Stat(filepath.Join(home, ".pi", "shevanio-ai", "persona.json")); !os.IsNotExist(err) {
...
func TestSyncPersonaPathsAndBackupTargetsTrackOnlyPiWorkspaceConfig(t *testing.T) {
	...
	want := filepath.Join(workspace, ".pi", "shevanio-ai", "persona.json")
	unwanted := filepath.Join(home, ".pi", "shevanio-ai", "persona.json")
...
// but managed OpenCode plugins are already installed on disk, `shevanio-ai sync`
...
func TestRunSyncRollbackRestoresClaudeEngramMigrationSource(t *testing.T) {
	...
	backups, err := os.ReadDir(filepath.Join(home, ".shevanio-ai", "backups"))
	if err != nil {
		t.Fatal(err)
	}
	if len(backups) != 1 {
...
func TestCodeGraphGuidanceSyncStepRefreshesOldMarkerWhenConfigured(t *testing.T) {
	...
	if !strings.Contains(text, "immediately run `shevanio-ai codegraph init --cwd <project-root>`") || !strings.Contains(text, "custom notes") {
...
func TestCodeGraphGuidanceSyncStepRestoresPartialInstallerFailure(t *testing.T) {
	...
	agentsPath := filepath.Join(home, ".config", "opencode", "AGENTS.md")
	mustWriteFile(t, settingsPath, []byte(`{}`))
	mustWriteFile(t, agentsPath, []byte("<!-- shevanio-ai:codegraph-guidance -->\nmanaged\n<!-- /shevanio-ai:codegraph-guidance -->\n"))
...
func TestCodeGraphGuidanceSyncStepRepairsCodexConfigOnlyGuidance(t *testing.T) {
	...
	text := string(body)
	for _, want := range []string{"<!-- shevanio-ai:codegraph-guidance -->", "immediately run `shevanio-ai codegraph init --cwd <project-root>`"} {
...
func TestCodeGraphGuidanceSyncStepCleansLegacyBlockWithoutCodeGraphCLI(t *testing.T) {
	...
	text := string(body)
	for _, stale := range []string{"<!-- CODEGRAPH_START -->", "<!-- CODEGRAPH_END -->", "old CodeGraph instructions", "<!-- shevanio-ai:codegraph-guidance -->"} {
		if strings.Contains(text, stale) {
...
func TestSyncRuntimeAddsCodeGraphStepsOnlyWhenSelected(t *testing.T) {
	home := t.TempDir()
	mustWriteFile(t, filepath.Join(home, ".config", "opencode", "opencode.json"), []byte(`{}`))
	mustWriteFile(t, filepath.Join(home, ".config", "opencode", "AGENTS.md"), []byte("<!-- shevanio-ai:codegraph-guidance -->\nold\n<!-- /shevanio-ai:codegraph-guidance -->\n"))
...
func writeManagedPiCodeGraphManifest(t *testing.T, home string) {
	t.Helper()
	manifestPath := filepath.Join(home, ".shevanio-ai", "pi-codegraph.json")
...
// shevanio-ai sync excludes it from the plan and does not adopt it."
//
...
func TestRunSyncExcludesUnmanagedLookalikeFile(t *testing.T) {
	...
	const lookalikeContent = "# My project AGENTS.md — NOT managed by shevanio-ai"
...
func TestRunSyncWithSelection_WritesExpectedFiles(t *testing.T) {
	...
	for name, content := range map[string]string{
		...
		if !strings.Contains(content, "shevanio-ai review status --cwd <repo> --contract shevanio-ai.review-integration/v2 --agent "+string(model.AgentOpenCode)+" --next-transition") {
...
func TestRunSyncRegeneratesPersonaBlockBetweenMarkers(t *testing.T) {
	...
	// Write a stale managed persona block — what an older version of shevanio-ai
...
func TestRunSyncWithSelectionPiUsesCanonicalPersonaForMissingPersonaField(t *testing.T) {
	...
	workspace := t.TempDir()
	t.Chdir(workspace)
	piPath := filepath.Join(workspace, ".pi", "shevanio-ai", "persona.json")
...
func TestRunSyncWithSelection_ExplicitEmptyPersistedPersonaFailsClosed(t *testing.T) {
	...
	originalPersona := "<!-- shevanio-ai:persona -->\nexisting valid persona\n<!-- /shevanio-ai:persona -->\n"
...
// `shevanio-ai sync` preserves Custom per-phase Codex model assignments from
