...
func TestReviewFacadeStartStagedProjectionFreezesOnlyIndex(t *testing.T) {
	...
	wantStagedEscape := "shevanio-ai review start --projection staged"
	wantBaseDiffEscape := fmt.Sprintf("shevanio-ai review start --base-ref %s --committed-only", base)
...
func TestReviewFacadeHelpAndFlatCompatibilityPathsRemainAvailable(t *testing.T) {
	for _, subcommand := range []string{"start", "finalize", "validate", "status", "recover"} {
		var output bytes.Buffer
		if err := RunReview([]string{subcommand, "--help"}, &output); err != nil || !strings.Contains(output.String(), "Usage: shevanio-ai review "+subcommand) {
	...
	for _, test := range []struct {
		...
		{RunReviewStart, "Usage: shevanio-ai review-start"}, {RunReviewStep, "Usage: shevanio-ai review-step"},
		{RunReviewResume, "Usage: shevanio-ai review-resume"}, {RunReviewValidate, "Usage: shevanio-ai review-validate"},
		{RunReviewBundleExport, "Usage: shevanio-ai review-bundle-export"}, {RunReviewBundleImport, "Usage: shevanio-ai review-bundle-import"},
	} {
		var output bytes.Buffer
...
func reviewFacadeUsageVerbs(t *testing.T, help string) map[string]bool {
	...
	const prefix = "Usage: shevanio-ai review <"
	const suffix = "> [flags]"
...
func TestEveryAdvertisedReviewFacadeVerbDispatches(t *testing.T) {
	...
	for _, verb := range verbs {
		t.Run(verb, func(t *testing.T) {
			...
			usage := "Usage: shevanio-ai review " + verb
...
func TestReviewBindSDDFeedsSelectedSDDStatusRuntime(t *testing.T) {
	...
	if !strings.Contains(output.String(), "shevanio-ai.sdd-review-binding/v1") {
...
func TestReviewFacadeFinalizeStateValidating(t *testing.T) {
	...
	t.Run("issue-1788: no evidence anywhere is a typed rejection, not a silent success", func(t *testing.T) {
		...
		for _, want := range []string{"shevanio-ai review capture-evidence", "shevanio-ai review finalize --lineage " + started.LineageID + " --captured-evidence"} {
