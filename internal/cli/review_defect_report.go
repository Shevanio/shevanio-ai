...
const reviewDefectReportIssuesURL = "https://github.com/Shevanio/shevanio-ai/issues/new/choose"

// reviewDefectReportDirName is the subdirectory under the repository's
// Git-common-dir "shevanio-ai" root (the same convention repository_locator.go
...
// reviewShevanioAIVersionAndCommit resolves the build's own version/commit the
...
// internal/app at startup. It is the same string `shevanio-ai --version` prints.
//
...
func reviewShevanioAIVersionAndCommit() (string, string) {
...
func newReviewDefectReport(input reviewDefectReportInput) reviewDefectReport {
	version, commit := reviewShevanioAIVersionAndCommit()
...
func (report reviewDefectReport) render() string {
	...
	fmt.Fprintf(&body, "Shevanio AI reached a tool-internal fault state (`%s`) that should never happen. %s\n\n",
	...
	fmt.Fprintf(&body, "## Shevanio AI Version\n\n%s (%s)\n\n", report.Version, report.Commit)
...
// <GitCommonDir>/shevanio-ai/defect-reports/ and returns the absolute path.
...
func writeReviewDefectReport(ctx context.Context, root string, report reviewDefectReport) (string, error) {
	...
	dir := filepath.Join(lease.Identity().GitCommonDir, "shevanio-ai", reviewDefectReportDirName)
