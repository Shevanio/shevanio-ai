...
// `shevanio-ai review <verb> ...` command out of a refusal, mirroring what an
...
func liftBacktickedReviewCommand(t *testing.T, message, verb string) []string {
	t.Helper()
	longest := []string{}
	for _, match := range regexp.MustCompile("`shevanio-ai ([^`]*)`").FindAllStringSubmatch(message, -1) {
		tokens := []string{}
	...
	t.Fatalf("message names no runnable `shevanio-ai review %s ...`: %q", verb, message)
	return nil
}
...
func dispatchNamedDiagnosisInRepo(t *testing.T, repo, message string) damagedStoreInspection {
	...
	if err := RunReview(args, &output); err != nil {
		t.Fatalf("the diagnosis the refusal named exits non-zero (named dead end): shevanio-ai %s: %v\n%s",
...
func dispatchNamedDiagnosis(t *testing.T, repo, message string) damagedStoreInspection {
	...
	if err := RunReview(tokens[1:], &output); err != nil {
		t.Fatalf("the diagnosis the denial named exits non-zero (named dead end): shevanio-ai %s: %v\n%s",
...
func TestDamagedEntryNamesItsOwnDamageKindAndDiagnosis(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if !strings.Contains(message, "shevanio-ai review inspect-authority") {
...
func TestReclaimRefusalOverTruncatedRecordNamesDiagnosisNotReconcile(t *testing.T) {
	...
	if strings.Contains(message, "shevanio-ai review reconcile-authority") {
