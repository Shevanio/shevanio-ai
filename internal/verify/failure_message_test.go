...
func TestVerificationIssuesMessageForCommandNamesTheGivenCommand(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			command: "shevanio-ai install --agent claude-code",
			want:    "Installation completed with verification issues. Run `shevanio-ai install --agent claude-code` to retry the failed checks.",
		},
		{
			name:    "sync retry command",
			command: "shevanio-ai sync",
			want:    "Installation completed with verification issues. Run `shevanio-ai sync` to retry the failed checks.",
		},
	}
