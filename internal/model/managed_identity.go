...
const (
	canonicalOrchestrator = "shevanio-orchestrator"
	canonicalDisplay      = "Shevanio"
)

// CanonicalManagedIdentity is the sole current managed identity.
var CanonicalManagedIdentity = newCanonicalManagedIdentity()

func newCanonicalManagedIdentity() ManagedIdentity {
	actor, _ := NormalizeOrchestratorRead(canonicalOrchestrator)
	persona, _ := NormalizePersonaRead(string(PersonaShevanio))
	preset, _ := NormalizePresetRead(string(PresetFullShevanio))
	return ManagedIdentity{
		Actor:    actor,
		Persona:  persona,
		Display:  canonicalDisplay,
		Resource: canonicalDisplay,
		Preset:   preset,
	}
}
