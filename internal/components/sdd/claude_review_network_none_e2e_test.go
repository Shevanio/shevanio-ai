...
const (
	...
	claudeRuntimeFakeKey = "shevanio-ai-network-none-mock-key"
)
...
func boundClaudeRuntime(t *testing.T) string {
	...
	if err != nil {
		if os.Getenv("SHEVANIO_AI_CLAUDE_RUNTIME_REQUIRED") == "1" {
...
func requireNetworkNone(t *testing.T) {
	...
	parent := os.Getenv("SHEVANIO_AI_PARENT_NETNS")
...
func TestClaudeReviewerTransportInNetworkNone(t *testing.T) {
	...
	agents, _ := json.Marshal(map[string]any{"shevanio-ai-review-transport-e2e": map[string]any{"description": "Exercises the generated Claude reviewer transport.", "prompt": strings.TrimSpace(parts[2]), "model": "sonnet", "tools": []string{}}})
	subject := "sha256:" + strings.Repeat("a", 64)
	const evidenceA = "- path_index: 0\n  path: parser.go\n  patch: |\n    -    return ErrMalformedPort\n    +    return nil"
	const evidenceB = "- path_index: 1\n  path: parser_test.go\n  patch: |\n    -    require.Error(t, err)\n    +    require.NoError(t, err)"
	prompt := `SHEVANIO_AI_REVIEW_BINDING {"lineage":"claude-runtime-e2e","target":"fixture","lens":"review-reliability","order":0,"revision":"1","repository_context":"opaque-fixture","subject_hash":"` + subject + `"}
SHEVANIO_AI_CLAUDE_REVIEW_CONTEXT
...
path_evidence:
` + evidenceA + "\n" + evidenceB + "\nSHEVANIO_AI_CLAUDE_REVIEW_CONTEXT_END"
