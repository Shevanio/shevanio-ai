...
type InjectOptions struct {
	...
	// <!-- shevanio-ai:strict-tdd-mode --> marker section is injected into
...
//
// Walking upward means users can run shevanio-ai from any subdirectory of their
...
func Inject(homeDir string, adapter agents.Adapter, sddMode model.SDDModeID, options ...InjectOptions) (InjectionResult, error) {
	...
	// findProjectRoot walks upward from WorkspaceDir so shevanio-ai can be
...
// extractManagedSection returns the content of one shevanio-ai managed section.
//
...
func extractManagedSection(content, sectionID string) string {
	open := "<!-- shevanio-ai:" + sectionID + " -->"
	closing := "<!-- /shevanio-ai:" + sectionID + " -->"
...
// stripOpenCodeBackgroundPolicy removes only the complete Shevanio AI-owned block.
...
const nativeReviewAuthorityRule = "7. **Authority rule**: read native review state with `shevanio-ai review status`" +
...
func ensurePreservedOpenCodeDelegationHardGates(prompt string) string {
	...
	// legacy heading belongs to shevanio-ai because users may have appended H4 or
...
`
	...
	if strings.Contains(prompt, "<!-- shevanio-ai:delegation-hard-gates-migration -->") &&
	...
	start := "<!-- shevanio-ai:delegation-hard-gates-migration -->"
	end := "<!-- /shevanio-ai:delegation-hard-gates-migration -->"
...
func removeRetiredWorkRoutingAuthorityRule(prompt string) string {
	...
	for _, line := range lines {
		retired := strings.Contains(line, "**Authority rule**") &&
			(strings.Contains(line, "shevanio-ai.work-") || strings.Contains(line, "WorkRun"))
		if retired {
			continue
		}
		kept = append(kept, line)
...
func migrateLegacyMandatoryWordingInDelegationHardGates(prompt string) string {
	const (
		startMarker = "<!-- shevanio-ai:delegation-hard-gates-migration -->"
		endMarker   = "<!-- /shevanio-ai:delegation-hard-gates-migration -->"
	)
...
func ensurePreservedOpenCodeReviewExecutionContract(prompt string) string {
	const (
		startMarker = "<!-- shevanio-ai:review-execution-contract-migration -->"
		endMarker   = "<!-- /shevanio-ai:review-execution-contract-migration -->"
...
`
	...
	start := "<!-- shevanio-ai:sdd-session-preflight-migration -->"
	end := "<!-- /shevanio-ai:sdd-session-preflight-migration -->"
	if startIdx := strings.Index(prompt, start); startIdx >= 0 {
		if relEndIdx := strings.Index(prompt[startIdx:], end); relEndIdx >= 0 {
			endIdx := startIdx + relEndIdx + len(end)
...
func ensureCodexSkillRegistryHook(hooksPath string) (bool, error) {
	...
	const command = `shevanio-ai skill-registry refresh --quiet --no-gitignore --cwd "$PWD" || true`
...
func ensureClaudeSkillRegistryHook(settingsPath string) (bool, error) {
	...
	const command = `shevanio-ai skill-registry refresh --quiet --no-gitignore --cwd "${CLAUDE_PROJECT_DIR:-$PWD}" || true`
	if claudeHookExists(root, command) {
		return false, nil
	}
...
// ManagedOpenCodePluginNames lists every OpenCode plugin shevanio-ai manages as
...
// installOpenCodePlugins copies the OpenCode-compatible plugins that shevanio-ai
...
func hasLegacyBareOrchestrator(content string) bool {
	markedIdx := strings.Index(content, "<!-- shevanio-ai:sdd-orchestrator -->")
	if markedIdx >= 0 {
...
//   - end at the next managed marker ("<!-- shevanio-ai:") if present, else EOF
...
func stripBareOrchestratorForFilePrompt(content string) string {
	if markedIdx := strings.Index(content, "<!-- shevanio-ai:sdd-orchestrator -->"); markedIdx >= 0 {
		prefix := content[:markedIdx]
	...
	if rel := strings.Index(content[start:], "<!-- shevanio-ai:"); rel >= 0 {
		end = start + rel
	}
...
func injectMarkdownSections(homeDir string, adapter agents.Adapter, legacyAssignments map[string]model.ClaudeModelAlias, phaseAssignments map[string]model.ClaudePhaseAssignment, renderOptions OrchestratorRenderOptions) (InjectionResult, error) {
	...
	if hasSDDOrchestrator(existing) && !strings.Contains(existing, "<!-- shevanio-ai:sdd-orchestrator -->") {
...
func renderClaudeModelAssignmentsSection(assignments map[string]model.ClaudePhaseAssignment) string {
	...
	b.WriteString("The Claude Code session model is controlled by Claude Code itself; Shevanio AI does not configure the main orchestrator model. This table applies only to Agent tool calls for SDD/Judgment-Day phase sub-agents, not generic delegation.\n\n")
