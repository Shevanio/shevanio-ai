...
func CodeGraphGuidanceMarkdown() string {
	return strings.Join([]string{
		...
		"- Do not use `shevanio-ai codegraph` as a general proxy. Its `init` command exists only to validate the project root before initialization; intelligence queries belong to the upstream CLI.",
		...
		"4. If `.codegraph/` is missing and CodeGraph is enabled/available, immediately run `shevanio-ai codegraph init --cwd <project-root>` once.",
...
func HasManagedCodeGraphGuidance(homeDir string) bool {
	for _, path := range CodeGraphGuidancePaths(homeDir) {
		...
		if strings.Contains(string(data), "<!-- shevanio-ai:"+codeGraphGuidanceSectionID+" -->") {
			return true
		}
	}
	return false
...
func hasCodeGraphGuidance(path string) bool {
	...
	return strings.Contains(content, "shevanio-ai:"+codeGraphGuidanceSectionID) ||
		(strings.Contains(content, "codegraph") && strings.Contains(content, "shevanio-ai codegraph init --cwd <project-root>"))
}
