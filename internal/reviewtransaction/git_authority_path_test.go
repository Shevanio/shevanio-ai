...
func TestReviewAuthorityRootRejectsLegacyOptionEchoWithoutMutation(t *testing.T) {
	...
	repo := initSnapshotRepo(t)
	original := gitCommandContext
	t.Setenv("SHEVANIO_AI_TEST_GIT_PATH_OUTPUT", base64.StdEncoding.EncodeToString([]byte("--path-format=absolute\n.git\n")))
	gitCommandContext = func(ctx context.Context, name string, args ...string) *exec.Cmd {
	...
	path := filepath.Join(repo, "shevanio-ai")
...
func TestResolveRepositoryRootRejectsUnrelatedAbsoluteOutput(t *testing.T) {
	...
	outside := t.TempDir()
	original := gitCommandContext
	t.Setenv("SHEVANIO_AI_TEST_GIT_PATH_OUTPUT", base64.StdEncoding.EncodeToString([]byte(outside+"\n")))
	gitCommandContext = func(ctx context.Context, name string, args ...string) *exec.Cmd {
...
func TestReviewAuthorityRootPreservesSymlinkedGitDirectory(t *testing.T) {
	...
	want := filepath.Join(externalGitDir, "shevanio-ai", "review-transactions")
...
func TestGitCommonDirectoryMismatchFailsBeforeAuthorityMutation(t *testing.T) {
	...
	for _, operation := range operations {
		t.Run(operation.name, func(t *testing.T) {
			...
			for _, path := range []string{filepath.Join(unrelated, "shevanio-ai"), filepath.Join(home, ".shevanio-ai")} {
				if _, err := os.Lstat(path); !os.IsNotExist(err) {
...
func TestReviewAuthorityRootRejectsMidQueryGitControlReplacement(t *testing.T) {
	...
	if _, err := os.Lstat(filepath.Join(repo, ".git", "shevanio-ai")); !os.IsNotExist(err) {
...
func TestGitAuthorityPathHelperProcess(t *testing.T) {
	if os.Getenv("SHEVANIO_AI_TEST_GIT_PATH_FAIL") == "1" {
		os.Exit(23)
	}
	encoded := os.Getenv("SHEVANIO_AI_TEST_GIT_PATH_OUTPUT")
