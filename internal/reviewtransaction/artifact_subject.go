...
const ArtifactSubjectSchemaV1 = "shevanio-ai.review-artifact-subject/v1"
const ArtifactSubjectSchema = "shevanio-ai.review-artifact-subject/v2"
...
func ChangedPathManifestDigest(entries []ChangedPathManifestEntry) (string, error) {
	...
	sum := sha256.Sum256(append([]byte("shevanio-ai.review-changed-path-manifest/v1\x00"), payload...))
...
func artifactSubjectHashV1(subject ArtifactSubject) string {
	...
	sum := sha256.Sum256(append([]byte("shevanio-ai.review-artifact-subject/v1\x00"), payload...))
...
func artifactSubjectHash(subject ArtifactSubject) string {
	...
	payload, _ := json.Marshal(preimage)
	sum := sha256.Sum256(append([]byte("shevanio-ai.review-artifact-subject/v2\x00"), payload...))
	return "sha256:" + hex.EncodeToString(sum[:])
}
