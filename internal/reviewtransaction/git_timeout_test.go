...
func TestRunGitBoundsLocalAndRemoteCommands(t *testing.T) {
	...
	t.Setenv("SHEVANIO_AI_GIT_TIMEOUT_HELPER", "sleep")
	...
	t.Setenv("SHEVANIO_AI_GIT_TIMEOUT_HELPER", "parent-exit")
	t.Setenv("SHEVANIO_AI_GIT_ESCAPE_MARKER", marker)
...
func TestRunGitDistinguishesAggregateDeadlineAndTypedExit(t *testing.T) {
	...
	t.Setenv("SHEVANIO_AI_GIT_TIMEOUT_HELPER", "exit")
...
func TestRunGitTimeoutHelper(t *testing.T) {
	switch os.Getenv("SHEVANIO_AI_GIT_TIMEOUT_HELPER") {
	case "parent-exit":
		child := exec.Command(os.Args[0], "-test.run=^TestRunGitTimeoutHelper$", "--")
		child.Env = append(os.Environ(), "SHEVANIO_AI_GIT_TIMEOUT_HELPER=mark")
		if err := child.Start(); err != nil {
			t.Fatal(err)
		}
	case "mark":
		time.Sleep(2 * time.Second)
		_ = os.WriteFile(os.Getenv("SHEVANIO_AI_GIT_ESCAPE_MARKER"), []byte("escaped"), 0o644)
	case "sleep":
