...
const (
	LedgerSchema         = "shevanio-ai.review-ledger/v1"
	CanonicalEmptyLedger = `{"schema":"shevanio-ai.review-ledger/v1","findings":[]}`
)
...
func validateCanonicalLedger(payload []byte, expectedFindings []Finding, suppliedHash string) (string, string, error) {
	...
	if envelope.Schema != LedgerSchema || envelope.Findings == nil {
		return "", "", errors.New("ledger requires shevanio-ai.review-ledger/v1 and an explicit findings array")
	}
