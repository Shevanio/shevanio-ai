...
func TestMaintenanceLockModesAndRelease(t *testing.T) {
	path := filepath.Join(t.TempDir(), "shevanio-ai", "review-transactions", "MAINTENANCE.lock")
...
func TestMaintenanceLockRejectsSymlinksAndStaleBytesAreNotOwnership(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "shevanio-ai", "review-transactions", "MAINTENANCE.lock")
...
func TestEnsureMaintenanceLockPathAcceptsCanonicalAbsolutePath(t *testing.T) {
	path := filepath.Join(t.TempDir(), "shevanio-ai", "REVIEW-MAINTENANCE.lock")
	if err := ensureMaintenanceLockPath(path); err != nil {
...
func TestMaintenanceLockIsReleasedWhenOwnerProcessExits(t *testing.T) {
	if os.Getenv("SHEVANIO_AI_MAINTENANCE_LOCK_EXIT_HELPER") == "1" {
		lock, err := acquireMaintenanceLock(context.Background(), os.Getenv("SHEVANIO_AI_MAINTENANCE_LOCK_EXIT_PATH"), maintenanceExclusive)
		if err != nil {
			t.Fatal(err)
		}
		_ = lock
	...
	command.Env = append(os.Environ(), "SHEVANIO_AI_MAINTENANCE_LOCK_EXIT_HELPER=1", "SHEVANIO_AI_MAINTENANCE_LOCK_EXIT_PATH="+path)
...
func TestMaintenanceLockRejectsSymlinkedAuthorityComponent(t *testing.T) {
	dir := t.TempDir()
	outside := t.TempDir()
	if err := os.Symlink(outside, filepath.Join(dir, "shevanio-ai")); err != nil {
		t.Fatal(err)
	}
	path := filepath.Join(dir, "shevanio-ai", "review-transactions", "MAINTENANCE.lock")
	if _, err := acquireMaintenanceLock(context.Background(), path, maintenanceShared); err == nil {
