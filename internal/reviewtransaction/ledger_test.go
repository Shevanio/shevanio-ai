...
func TestCanonicalLedgerValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing schema", payload: `{"findings":[]}`, findings: []Finding{}, wantErr: "requires shevanio-ai.review-ledger/v1"},
		{name: "wrong schema", payload: `{"schema":"shevanio-ai.review-ledger/v2","findings":[]}`, findings: []Finding{}, wantErr: "requires shevanio-ai.review-ledger/v1"},
		{name: "missing findings", payload: `{"schema":"shevanio-ai.review-ledger/v1"}`, findings: []Finding{}, wantErr: "explicit findings array"},
		{name: "unknown field", payload: `{"schema":"shevanio-ai.review-ledger/v1","findings":[],"extra":true}`, findings: []Finding{}, wantErr: "unknown field"},
		{name: "orchestration metadata is not native", payload: `{"schema":"shevanio-ai.review-ledger/v1","findings":[{"id":"R1-001","severity":"CRITICAL","evidence_class":"deterministic","status":"open"}]}`, findings: []Finding{{ID: "R1-001", Severity: "CRITICAL"}}, wantErr: "unknown field"},
		{name: "trailing newline", payload: CanonicalEmptyLedger + "\n", findings: []Finding{}, wantErr: "not the canonical compact JSON"},
		{name: "alternate field order", payload: `{"findings":[],"schema":"shevanio-ai.review-ledger/v1"}`, findings: []Finding{}, wantErr: "not the canonical compact JSON"},
