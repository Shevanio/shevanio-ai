...
const finalizeAttemptJournalSchema = "shevanio-ai.review-finalize-attempt-journal/v1"
...
func FinalizeAttemptValueDigest(domain string, value any) string {
	...
	sum := sha256.Sum256(append([]byte("shevanio-ai.finalize-input/"+domain+"\x00"), payload...))
