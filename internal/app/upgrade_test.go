...
// TestRunArgs_UpgradeDryRun verifies that `shevanio-ai upgrade --dry-run` runs without
...
// TestRunArgs_UpgradeNoArgs runs `shevanio-ai upgrade` without flags.
...
// TestRunArgs_UpgradeToolFilter verifies that `shevanio-ai upgrade engram` filters
...
func TestRenderUpgradeReport_PerToolSemantics_Deterministic(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantContains:   []string{"shevanio-ai", "[--]", "source build"},
			...
			results: []upgrade.ToolUpgradeResult{
				{
					...
					NewVersion: "1.5.0",
					Status:     upgrade.UpgradeSkipped,
					ManualHint: "Download from https://github.com/Shevanio/shevanio-ai/releases",
				},
			},
			wantContains:   []string{"shevanio-ai", "manual update required", "github.com", "[--]"},
			wantNotContain: []string{"[!!]", "FAILED"},
		},
		{
