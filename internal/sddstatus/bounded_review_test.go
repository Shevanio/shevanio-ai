...
func TestResolveArchiveRequiresApprovedExactReviewReceipt(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			mutate: func(t *testing.T, changeRoot string, _ reviewtransaction.Receipt, _ reviewtransaction.GateRequest) {
				write(t, filepath.Join(changeRoot, "reviews", "ledger.json"), "{\"schema\":\"shevanio-ai.review-ledger/v1\",\"findings\":[{\"id\":\"stale\"}]}")
			},
			wantGate: reviewtransaction.GateAllow, wantArchive: DependencyReady, wantNext: "archive",
		},
		{
...
func TestNativeReceiptDiscoveryRejectsMultipleGoverningLineagesAsAmbiguous(t *testing.T) {
	...
	if !strings.Contains(status.ReviewGate.Reason, "shevanio-ai review bind-sdd") {
...
func TestEngramReviewArtifactDiscoveryRetainsOnlyExactNames(t *testing.T) {
	observations := []engramObservation{
		{Title: "sdd/bounded/review/chain-bundle", Content: "exact bundle", Project: "shevanio-ai", Scope: "project"},
		{Title: "sdd/bounded/review/chain-bundles", Content: "plural", Project: "shevanio-ai", Scope: "project"},
		{Title: "sdd/bounded/review/chain-bundle/extra", Content: "nested", Project: "shevanio-ai", Scope: "project"},
		{Title: "sdd/bounded/review/arbitrary", Content: "arbitrary", Project: "shevanio-ai", Scope: "project"},
	}

	changes := collectEngramChanges(observations, "shevanio-ai")
	if !reflect.DeepEqual(changes, []string{"bounded"}) {
		t.Fatalf("collectEngramChanges() = %v, want exact chain-bundle change", changes)
	}
	artifacts := engramArtifactsForChange(observations, "shevanio-ai", "bounded")
...
func TestResolveRejectsForeignCompactAuthorityForStaleVerifyEvidence(t *testing.T) {
	...
	dispatcher := RenderDispatcherMarkdown(status)
	for _, want := range []string{"### Next Review Operation", "shevanio-ai review start", "shevanio-ai review validate --gate post-apply"} {
