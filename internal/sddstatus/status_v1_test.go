...
func TestStatusRenderersEmbedOnlyStatusV1Projection(t *testing.T) {
	...
	for name, output := range rendered {
		t.Run(name, func(t *testing.T) {
			...
			if !strings.Contains(output, `"schemaName": "shevanio-ai.sdd-status"`) {
...
func TestParseCommandArgsRejectsUnsupportedContractReadOnly(t *testing.T) {
	tests := []struct {
		...
		{name: "unknown", args: []string{"change", "--contract", "shevanio-ai.work-status/v1"}, want: `unsupported sdd-status contract "shevanio-ai.work-status/v1"`},
