...
// `shevanio-ai sdd-attempt status` command is rejected by the CLI for missing
...
func TestRuntimeLedgerErrorsNameTheStatusRoute(t *testing.T) {
	const pointer = "run `shevanio-ai sdd-attempt status --cwd <repo> --change <change>` — its next_action names the continuation"
