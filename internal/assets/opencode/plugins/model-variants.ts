/**
 * model-variants
 * Exports per-model variant (effort level) data for shevanio-ai.
 *
 ...
 * ~/.shevanio-ai/cache/model-variants.json. shevanio-ai reads this file
...
export const ModelVariantsPlugin: Plugin = async (input) => {
  async function refreshVariantsCache() {
    ...
    try {
      ...
      const cacheDir = path.join(homedir(), ".shevanio-ai", "cache")
