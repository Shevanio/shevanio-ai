...
Build a complete `verify-report` in memory as exact candidate bytes. Count authoritative requirements and scenarios, then run `shevanio-ai sdd-verify-validate` on those bytes before any OpenSpec or Engram write.
