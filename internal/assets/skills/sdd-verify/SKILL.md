...
- Build the complete report as exact candidate bytes, then run `shevanio-ai sdd-verify-validate` with authoritative spec counts before any OpenSpec or Engram write. If the validator is unavailable or denies admission, make zero writes and leave the prior report untouched; otherwise persist the same bytes, including a valid `fail`.
...
7. Run test, build/type-check, and coverage commands when available. For full spec verification, preserve shevanio-ai's stricter runtime evidence: source inspection alone does not prove spec scenario compliance.
