...
// organicNamedCommand extracts the single `shevanio-ai ...` command a refusal
...
func organicNamedCommand(t *testing.T, message string) ([]string, []string) {
	...
	index := strings.Index(message, "shevanio-ai ")
	if index < 0 {
	...
	if len(fields) < 2 || fields[0] != "shevanio-ai" {
		t.Fatalf("refusal named %q, which is not a shevanio-ai command:\n%s", tail, message)
	}
	placeholders := []string{}
...
func TestOrganicEscalatedGateDenialNamesARunnableContinuation(t *testing.T) {
	...
	if stdout, recoverStderr, recoverErr := harness.gentleAllowFailure(arguments...); recoverErr != nil {
		t.Fatalf("the continuation the escalated denial named failed: shevanio-ai %v: %v\nstdout:\n%s\nstderr:\n%s",
...
func TestOrganicEscalatedRecoveryRefusalNamesWhatToChange(t *testing.T) {
	...
	fixOrganicEscalatedCandidate(harness)
	if stdout, recoverStderr, recoverErr := harness.gentleAllowFailure(arguments...); recoverErr != nil {
		t.Fatalf("the command the unchanged-target refusal named failed: shevanio-ai %v: %v\nstdout:\n%s\nstderr:\n%s",
			arguments, recoverErr, stdout, recoverStderr)
	}
