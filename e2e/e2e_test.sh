#!/usr/bin/env bash
# e2e_test.sh — End-to-end tests for shevanio-ai installer
#
...
if [ -z "$BINARY" ]; then
    echo "ERROR: shevanio-ai binary not found. Build it first."
    exit 1
fi
...
test_version_command() {
    ...
    if echo "$output" | grep -q "shevanio-ai"; then
...
test_cc_engram_injection() {
    ...
    if $BINARY install --agent claude-code --component engram --persona neutral 2>&1; then
        ...
        assert_file_contains "$HOME/.claude/CLAUDE.md" "shevanio-ai:engram-protocol" "CLAUDE.md has engram-protocol section marker"
...
test_cc_sdd_injection() {
    ...
    if $BINARY install --agent claude-code --component sdd --persona neutral 2>&1; then
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "shevanio-ai:sdd-orchestrator" "CLAUDE.md has SDD section marker"
...
test_cc_persona_gentleman() {
    ...
    if $BINARY install --agent claude-code --component persona --persona gentleman 2>&1; then
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "shevanio-ai:persona" "CLAUDE.md has persona section marker"
...
test_oc_engram_injection() {
    ...
    if $BINARY install --agent opencode --component engram --persona neutral 2>&1; then
        ...
        assert_file_contains "$agents_md" 'shevanio-ai:engram-protocol' "AGENTS.md has engram-protocol section"
...
test_full_preset_claude_code() {
    ...
    if $BINARY install --agent claude-code --component sdd --component persona --component skills --component context7 --component permissions --component theme --preset full-gentleman --persona gentleman 2>&1; then
        ...
        assert_file_contains "$claude_md" "shevanio-ai:sdd-orchestrator" "Has SDD section"
        assert_file_contains "$claude_md" "shevanio-ai:persona" "Has persona section"
...
test_minimal_preset_opencode_only_engram_no_persona() {
    ...
    if $BINARY install --agent opencode --preset minimal --persona custom 2>&1; then
        ...
        if [ -f "$agents_md" ]; then
            assert_file_not_contains "$agents_md" "shevanio-ai:persona" "No persona marker in minimal preset"
...
test_minimal_preset_claude_only_engram() {
    ...
    if $BINARY install --agent claude-code --preset minimal --persona custom 2>&1; then
        ...
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "shevanio-ai:engram-protocol" "Engram protocol section"

        # SDD should NOT be in CLAUDE.md
        assert_file_not_contains "$HOME/.claude/CLAUDE.md" "shevanio-ai:sdd-orchestrator" "No SDD in minimal"
        # Persona should NOT be in CLAUDE.md
        assert_file_not_contains "$HOME/.claude/CLAUDE.md" "shevanio-ai:persona" "No persona in minimal"
...
test_ecosystem_both_agents() {
    ...
    if $BINARY install --agent claude-code --agent opencode --component sdd --component skills --component context7 --preset ecosystem-only --persona neutral 2>&1; then
        ...
        assert_file_contains "$HOME/.claude/CLAUDE.md" "shevanio-ai:sdd-orchestrator" "Claude has SDD"
...
test_edge_theme_not_in_presets() {
    ...
    if $BINARY install --agent claude-code --component theme --persona neutral 2>&1; then
        ...
        if [ -f "$HOME/.claude/CLAUDE.md" ]; then
            local sections
            sections=$(grep -o '<!-- shevanio-ai:[a-z-]* -->' "$HOME/.claude/CLAUDE.md" | sort -u | tr '\n' ' ')
            if [ "$(printf '%s' "$sections" | xargs)" = "<!-- shevanio-ai:agent-routing -->" ]; then
...
test_edge_persona_switch_preserves_sections_opencode() {
    ...
    assert_file_contains "$agents_md" "shevanio-ai:engram-protocol" "Engram section present before switch"
    ...
    assert_file_contains "$agents_md" "shevanio-ai:engram-protocol" "Engram section survived persona switch"
...
test_windsurf_persona_and_sdd_content() {
    ...
    if $BINARY install --agent windsurf --component persona --component sdd --persona gentleman 2>&1; then
        ...
        assert_file_contains "$rules" "shevanio-ai:sdd-orchestrator" "SDD orchestrator marker present"
        ...
        assert_file_contains "$rules" "shevanio-ai:engram-protocol" "Engram protocol marker present"
...
test_backup_created_on_install() {
    ...
    if $BINARY install --agent opencode --component permissions --persona neutral 2>&1; then
        local backup_count
        backup_count=$(find "$HOME/.shevanio-ai/backups" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l | tr -d ' ')
...
test_backup_contains_original_files() {
    ...
    if $BINARY install --agent opencode --component permissions --persona neutral 2>&1; then
        local latest_backup
        latest_backup=$(find "$HOME/.shevanio-ai/backups" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | sort | tail -1)
