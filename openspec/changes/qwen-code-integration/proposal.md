...
Add Qwen Code (by Alibaba/Alibaba Cloud) as a fully supported AI coding agent in the Shevanio AI ecosystem, with parity to Gemini CLI's feature set. Qwen Code is an emerging agent in the AI-assisted development space with robust sub-agent capabilities, slash commands, and a `settings.json`-based config model that aligns well with the existing Shevanio AI adapter pattern.
...
- Sub-agent injection (Qwen's `task` sub-agent is intrinsic to the agent, not configured by Shevanio AI)
...
- [ ] `shevanio-ai install --agent qwen-code --dry-run` shows correct plan
