...
When `sdd.Inject()` is called with a Qwen Code adapter, the SDD orchestrator content MUST be injected into `~/.qwen/QWEN.md` using `<!-- shevanio-ai:sdd-orchestrator -->` markers.

#### Scenario: SDD orchestrator injection

- GIVEN a fresh home directory with no `~/.qwen/`
- WHEN `Inject(homeDir, qwenAdapter, "")` is called
- THEN `~/.qwen/QWEN.md` is created and contains `<!-- shevanio-ai:sdd-orchestrator -->` markers
