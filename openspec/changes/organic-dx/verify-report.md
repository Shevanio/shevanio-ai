...
| `go test ./e2e/organicruntime -count=1 -timeout=15m` | 0 | `ok github.com/shevanio/shevanio-ai/e2e/organicruntime 79.228s` |
...
**S-3 — `targetResolution` counts as deterministically-stale.** In the *enabled*-mode framing extension, a composition containing only target-resolution failures now says "no terminal review receipt governs this candidate; review it directly with `shevanio-ai review start`" — but a repository that could not resolve a publication boundary may not be able to start either. Blocking is unchanged, so this is framing, not safety.
