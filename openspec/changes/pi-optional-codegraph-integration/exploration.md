...
- `internal/components/uninstall/service.go` — must include only Shevanio AI-owned Pi CodeGraph files/settings in backup and cleanup operations.
...
### Approaches
1. **Pi-specific optional integration managed by Shevanio AI** — Add a narrowly scoped CodeGraph-for-Pi provisioner that installs the CodeGraph CLI/MCP/extension, writes only Shevanio AI-owned Pi configuration/guidance, discovers compatible child assets, injects tools only where supported, injects lazy-init guidance into every child, verifies those children directly, reapplies after Pi asset sync, and removes its owned artifacts on uninstall.
