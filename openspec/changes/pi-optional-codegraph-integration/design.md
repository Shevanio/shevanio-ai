...
Add a Pi-specific reconciler owning a minimal Pi MCP key, child overlays/marker blocks, and `~/.shevanio-ai/pi-codegraph.json`. It never changes `gentle-pi`; install and sync invoke one transaction after refreshes.
