...
Grounded against the POST-W6 tree `/home/gentleman/work/shevanio-ai-worktrees/rdd-wave6 @ 40176a8f` (read-only; pending-verify caveat applies). Every `file:line` below was read at design time, not inferred.
...
| 2 | `shadowObservationEnvVar` = `SHEVANIO_AI_RDD_SHADOW` | `shadow_observer.go` | `shadow_observer_test.go`, `new_lineage_switch_identity_test.go` | S2 |
...
| 7 | W4 task 8.5 (relaunch-bound loss) | CLOSE AS OUT-OF-SCOPE FOR W7 — not `wontfix-superseded`. Standing follow-up owned by the admission flow | Close as superseded, or build a provider-side replacement here | #10166 confirmed zero cross-references: the plugin's `SHEVANIO_AI_REVIEW_BINDING` capture-task bookkeeping is structurally unconnected to Go-side legacy mutation. W7 retires nothing in that file, so the "confirm a replacement first" precondition never triggers. Calling it superseded would falsely imply the bounded-retry property is now covered — it is not. |
