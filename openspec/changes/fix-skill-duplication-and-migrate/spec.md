...
- After `shevanio-ai install` or `shevanio-ai sync` on any `~/.claude/` directory, none of the 11 SDD SKILL.md entries MUST appear as user-invocable items in the Claude Code `/` picker.
...
- GIVEN a fresh `~/.claude/skills/` directory with no prior shevanio-ai install
- WHEN `shevanio-ai install` runs with the Claude adapter selected
...
- GIVEN a Claude Code v2.1.131+ session with the new SKILL.md files installed via `shevanio-ai sync`
...
- After `shevanio-ai install`, each refactored skill directory on disk MUST contain both `SKILL.md` and its `references/` subdirectory.
...
- WHEN `shevanio-ai install` runs for the Claude adapter
...
- Migration logic — existing installs pick up new content automatically on next `shevanio-ai sync`.
