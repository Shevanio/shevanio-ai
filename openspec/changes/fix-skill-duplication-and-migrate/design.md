...
| 2 | A user manually edits an installed `~/.claude/skills/sdd-*/SKILL.md`, then runs `shevanio-ai sync` and the frontmatter is overwritten | Low | Low | Out of scope for this change — sync's overwrite policy for embedded assets is deliberate and pre-existing. Document elsewhere if it becomes a real complaint. |
