...
Success looks like: in Claude Code v2.1.131 with the next shevanio-ai sync, each `sdd-*` slash command appears EXACTLY ONCE in `/`, and that single entry comes from `~/.claude/commands/sdd-*.md`. No path migration, no orphan files, no cross-adapter ripple.
...
- Migration logic. Existing installs pick up the new frontmatter automatically on the next `shevanio-ai sync` — same path, new content.
...
4. **Smoke test on a real install**: after `shevanio-ai sync` against a Claude Code v2.1.131 install, type `/` and verify each of `sdd-apply`, `sdd-archive`, `sdd-explore`, `sdd-init`, `sdd-onboard`, `sdd-verify` appears exactly once. Capture in the verify phase.
