...
`contracts/work-routing/` does not exist on `origin/main`; shevanio-ai HEAD carries 37 files under `contracts/work-routing/v1/`. Pi vendors 15 of them: 4 byte-identical, 11 matching an earlier branch commit, 22 never vendored. Vendor drift does not soften the impact — deleting the eight `work-*` commands still breaks Pi's live consumers `lib/native-work-cli.ts`, `lib/work-routing-v1.ts`, `lib/work-routing-consumer.ts`, and `extensions/shevanio-ai.ts`. Treat it as a breaking change and record it in the release ledger.
