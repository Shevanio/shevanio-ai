...
- THEN the agent runs `shevanio-ai review mode disable` without arguing or proposing an alternative first
...
- THEN it names `shevanio-ai review mode enable|disable|status` and the read-only nature of `status`
