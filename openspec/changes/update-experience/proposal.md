...
- `self-update`: CLI prompt becomes default (env gate removed); upgrade executor honors `SHEVANIO_AI_CHANNEL`.
...
- **State store**: add fields to `InstallState` (`~/.shevanio-ai/state.json`): `last_update_check`, `pending_sync` (deferred-sync flag). No skip/snooze fields by decision.
...
- **Channel**: upgrade executor reads `SHEVANIO_AI_CHANNEL` and routes beta to `@main`, stable to latest release.
...
| upgrade executor (`strategy.go` area) | Modified | Honor `SHEVANIO_AI_CHANNEL` |
...
3. **Channel fix** — upgrade executor honors `SHEVANIO_AI_CHANNEL`.
...
- One transition shevanio-ai release shipping this work (bootstrap, below).

## Bootstrap / Migration Plan

New behavior only exists in clients that already updated (chicken-and-egg). CLI users mostly auto-migrate — `selfUpdate` of the shevanio-ai binary works and is unaffected by the engram pin. TUI-only users see only a passive banner today and must act once manually. The version-independent escape hatch is the install script (`irm/curl … | iex/sh`), which always pulls latest regardless of installed logic. Plan: cut ONE transition shevanio-ai release with this work, advertise the one-time manual update out-of-band (Discord / README / release notes); from then on the new prompt + manifest carry the load. Transition cost is exactly one more shevanio-ai release; afterward engram releases reach users via runtime fetch with zero shevanio-ai release.

## Success Criteria

- [ ] An engram bump reaches users with ZERO shevanio-ai release (runtime fetch).
...
- [ ] `shevanio-ai upgrade` installs from the channel-correct source (beta → `@main`).
