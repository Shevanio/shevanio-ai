...
`last_update_check` MUST be persisted as a field in `~/.shevanio-ai/state.json`. It MUST be a timestamp (RFC 3339 or Unix epoch). Older binaries that do not know this field MUST be able to load the state file without error (additive field, backward-compatible).
