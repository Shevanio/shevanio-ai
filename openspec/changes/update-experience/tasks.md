...
- [ ] 3.4 `internal/update/upgrade/strategy.go` — update `engramDownloadFn` signature to accept channel; `engramBinaryUpgrade` reads `SHEVANIO_AI_CHANNEL` via `cli.ResolveInstallChannel` and passes channel to `engramDownloadFn`
...
- [ ] 4.2 `internal/app/selfupdate_test.go` (new or existing) — add test: after successful `shevanio-ai` self-upgrade, `PendingSync = true` is written to state before process exit
...
- [ ] 4.5 `internal/app/selfupdate.go` — after `upgradeExecute` confirms `shevanio-ai` succeeded, call `state.Read` + set `PendingSync = true` + `state.Write` before calling `restartAfterShevanioAIUpgrade`
- [ ] 4.6 `internal/app/selfupdate.go` `restartAfterShevanioAIUpgrade` — converge Unix + Windows: drop `goOS() == "windows"` branch; always print "Updated to vX — restart shevanio-ai…" and return (no re-exec); remove `reExec` var and `syscall` import if unused
...
### Phase 1 — Red (failing tests)
- [ ] 5.1 `internal/app/selfupdate_test.go` — add test: `selfUpdate` calls `promptFn` unconditionally when update is available (without `SHEVANIO_AI_CONFIRM_UPDATE`); test `SHEVANIO_AI_CONFIRM_UPDATE` env set to "1" does NOT gate the prompt (env is ignored)
...
- [ ] 5.5 `internal/app/selfupdate.go` — add `--yes` flag handling: accept `yesFlag bool` param or read `SHEVANIO_AI_YES=1`; when set, substitute `promptFn` with a stub that returns `(true, nil)`
...
### Phase 3 — Refactor
- [ ] 5.7 Remove `envConfirmUpdate` from constant block entirely; update any test that set it; add a comment "SHEVANIO_AI_CONFIRM_UPDATE removed — prompt is now unconditional"
...
### Phase 2 — Green (implementation)
- [ ] 7.6 `internal/update/advisory.go` (create) — define `Advisory{Message, Severity, URL string}`; implement `FetchAdvisory(ctx context.Context) (Advisory, bool)` with 2s timeout, GET to advisory tag asset URL (`https://github.com/Shevanio/shevanio-ai/releases/download/advisory/advisory.json`), JSON decode, fail-open on any error
