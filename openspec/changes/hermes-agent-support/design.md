...
- Inject context7 + engram MCP into `~/.hermes/config.yaml` idempotently, without destroying
  user content or comments outside shevanio-ai-managed server blocks.
...
| **Rationale** | Marker sections are battle-tested (Claude Code, OpenClaw). engram, SDD, strict-TDD, and persona each inject their own `<!-- shevanio-ai:ID -->` section. Global path = standard flow. |
...
Hermes uses `StrategyMarkdownSections`, so persona is injected as a `<!-- shevanio-ai:persona -->`
...
| | |
|---|---|
| **Choice** | Write shevanio-ai `SKILL.md` files into `~/.hermes/skills/` (`SupportsSkills()=true`) using the standard SDD skills flow, but record a **documented assumption** that Hermes tolerates shevanio-ai's `SKILL.md` frontmatter format. The SDD orchestrator instructions live in `SOUL.md` regardless, so even if Hermes ignores or rejects unknown skill files, the orchestrator protocol is still loaded. |
...
shevanio-ai-managed block, re-append a fresh block, return content ending in a single trailing `\n`.
...
// enabling shevanio-ai to preserve a command already written for a server
...
- Content **outside** any shevanio-ai-managed server block is preserved verbatim, including:
  user comments (`# ...`), other top-level keys, and other MCP servers (user-defined or a sibling
  shevanio-ai server).
- Comments **inside** a managed server block are shevanio-ai-owned and may be lost on re-write
...
Pipeline (all paths use ~/.hermes/ — global, no workspace dir):
  ├── persona:  inject <!-- shevanio-ai:persona --> into SOUL.md
...
| Comment loss inside managed blocks | Low | Documented: managed blocks are shevanio-ai-owned; content outside preserved (golden #6) |
...
| Integration | persona inject into SOUL.md | `t.TempDir()`; assert `<!-- shevanio-ai:persona -->` section; assert engram/SDD sections coexist without duplication |
