...
The adapter MUST report `SupportsAutoInstall() = false`. Hermes is detect-only; shevanio-ai will never attempt to install it automatically.
...
`InstallCommand()` MUST return an error indicating Hermes is not auto-installable, with a message directing the user to install Hermes manually. It MUST NOT return a command slice that shevanio-ai will execute.
...
`SystemPromptStrategy()` MUST return `model.StrategyMarkdownSections`. SOUL.md content is managed via `<!-- shevanio-ai:... -->` marker-based section injection without replacing user content.
...
When shevanio-ai configures Hermes, the `context7` MCP server block MUST be written to `~/.hermes/config.yaml` under `mcp_servers:`.
...
When shevanio-ai configures Hermes with engram enabled, the `engram` MCP server block MUST be written to `~/.hermes/config.yaml` under `mcp_servers:`.
...
- WHEN shevanio-ai reconfigures Hermes (engram injection runs again)
...
- WHEN shevanio-ai reconfigures Hermes
...
When `sdd.Inject()` is called with the Hermes adapter, the SDD orchestrator content MUST be written into `~/.hermes/SOUL.md` using `<!-- shevanio-ai:sdd-orchestrator -->` markers, without replacing content outside the markers.
...
- THEN `~/.hermes/SOUL.md` is created and contains `<!-- shevanio-ai:sdd-orchestrator -->` markers with orchestrator content between them
...
- THEN user content outside the `<!-- shevanio-ai:sdd-orchestrator -->` markers is preserved verbatim
- AND the orchestrator block is inserted or updated within the markers

#### Scenario: Orchestrator injection — idempotent

- GIVEN `~/.hermes/SOUL.md` already contains the `<!-- shevanio-ai:sdd-orchestrator -->` block
...
The strict-TDD instructions MUST be injected into `~/.hermes/SOUL.md` via `<!-- shevanio-ai:strict-tdd -->` markers using the standard markdown-sections flow.
...
When persona injection runs for Hermes, the persona content MUST be written into `~/.hermes/SOUL.md` using the standard `StrategyMarkdownSections` flow (same `<!-- shevanio-ai:persona -->` markers).
...
- THEN `~/.hermes/SOUL.md` contains the Hermes-specific gentleman persona content within `<!-- shevanio-ai:persona -->` markers

### Requirement: Persona Language Contract — Technical Artifacts in English

Technical artifact content (code comments, SOUL.md injected blocks, skill references, identifiers) generated by shevanio-ai for Hermes MUST default to English, regardless of persona voice or conversation language. The persona governs agent tone in SOUL.md replies, not the language of embedded technical content.
