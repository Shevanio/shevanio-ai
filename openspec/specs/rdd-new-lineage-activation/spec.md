...
The activation switch MUST be a dedicated environment variable, default OFF, distinct in identity and meaning from `SHEVANIO_AI_RDD_SHADOW` and from the user-owned RDD kill switch (adopted default). When off, every `start` MUST take the legacy path and create no new-lineage artifact.
...
- GIVEN the activation switch, `SHEVANIO_AI_RDD_SHADOW`, and the RDD kill switch
