...
Define observable behavior for how shevanio-ai renders and injects the Engram save-trigger protocol into each supported adapter's context, so the same normative content is not duplicated across channels within a single model session, while never breaking `engram setup` on older binaries or dropping adapter-specific content.
...
- WHEN shevanio-ai renders the Codex AGENTS.md section, `model_instructions_file`, and compact prompt
...
- WHEN shevanio-ai injects the Engram protocol section for that adapter
...
- WHEN shevanio-ai invokes `engram setup`
...
- WHEN shevanio-ai performs verbosity detection and forwarding around `engram setup`
