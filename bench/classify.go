...
// Observation is everything the benchmark can see about ONE shevanio-ai
...
var commandStart = regexp.MustCompile("(?:^|[\\s'\"`(\\[])shevanio-ai(?:[\\s'\"`)\\]]|$)")
...
// immediately runnable shevanio-ai command: the product name, at least one
...
// a template like `shevanio-ai review validate --gate <gate>` that the identical
...
// unsupportedPatterns are the shapes a shevanio-ai build emits when the CLI
...
//  3. text names a runnable `shevanio-ai <verb> ...`  -> in_band
...
//     it simply is not expressible as a `shevanio-ai` command — because it needs
...
// consentNotices are the exact stderr strings shevanio-ai emits when it would
