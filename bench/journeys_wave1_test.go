...
func TestWaveReviewInvocationArgs(t *testing.T) {
	got, err := waveReviewInvocationArgs("shevanio-ai review start --contract=shevanio-ai.review-integration/v2 --consent=relay")
	if err != nil {
		t.Fatal(err)
	}
	if joined := strings.Join(got, "\x00"); joined != "review\x00start\x00--contract=shevanio-ai.review-integration/v2\x00--consent=relay" {
		t.Fatalf("review invocation args = %q", joined)
	}
	for _, invalid := range []string{"", "other review start", "shevanio-ai status"} {
