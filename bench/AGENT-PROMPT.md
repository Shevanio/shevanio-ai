...
|---|---|
| `<BENCH-PATH>` | Path to the compiled `shevanio-ai-bench` binary (`cd bench && go build ./...`) |
| `<SHEVANIO-AI-PATH>` | Path to the `shevanio-ai` binary you want to measure |
...
```
You are an external tester evaluating shevanio-ai. You are NOT a developer of
this tool and you are NOT going to fix anything or open issues. You only test
and report.

## The rule that does not bend

Do NOT read shevanio-ai's source code. Do not open its repository, do not search
...
## Step 1 — Start the recording
  ...
  <BENCH-PATH> record --binary <SHEVANIO-AI-PATH> --out /tmp/session-guide.jsonl
...
this:
  ...
  CI=1 PATH=/tmp/session-guide.jsonl.shim:$PATH shevanio-ai <whatever>
...
Verify the shim before continuing:
  ...
  PATH=/tmp/session-guide.jsonl.shim:$PATH which shevanio-ai
...
Also write down the exact version under test:
  ...
  CI=1 PATH=/tmp/session-guide.jsonl.shim:$PATH shevanio-ai --version

Expected while recording: `shevanio-ai doctor` reports two copies of shevanio-ai on
...
- **Do not use pipes to capture output if you are going to look at the exit
  ...
  pipeline. `shevanio-ai ... | tee log` always gives 0 even if the binary failed.
  Use redirection to a file: `shevanio-ai ... > out.txt 2> err.txt` and then
  `echo $?`.
