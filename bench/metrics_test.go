...
func TestAccumulatorCountsEveryDimension(t *testing.T) {
	...
	consentNotice := "Shevanio AI reviewed this change without asking, because this session has no terminal to answer on.\n"
...
func TestAnalyzeSessionUsesTheSameClassifier(t *testing.T) {
	session := []SessionRecord{
		{Argv: []string{"review", "start"}, ExitCode: 0, Stdout: `{"state":"reviewing"}`,
			Stderr:         "Shevanio AI reviewed this change without asking, because this session has no terminal to answer on.\n",
			StdoutCaptured: true, StderrCaptured: true},
		{Argv: []string{"review", "finalize"}, ExitCode: 1,
			Stderr:         "Error: capture it first with `shevanio-ai review capture-evidence`\n",
			StdoutCaptured: true, StderrCaptured: true},
