...
// SessionRecord is one recorded shevanio-ai invocation from a real agent
...
var knownEnvelopes = []string{
	"shevanio-ai.review-gate-result/v1",
	"shevanio-ai.review-integration.status/v2",
	"shevanio-ai.review-capture-preflight/v1",
	"shevanio-ai.review-result-artifact/v2",
	"shevanio-ai.review-verification-evidence/v2",
	"shevanio-ai.review-mode/v1",
	"shevanio-ai.rdd-mode-status/v1",
}
...
func setupRecording(realBinary, outPath string) error {
	...
	shim := filepath.Join(shimDir, "shevanio-ai")
	script := "#!/bin/sh\n" +
	...
	fmt.Printf("2. Run your agent through the testing guide as usual. Every shevanio-ai\n")
...
func runShim(real, logPath string, args []string) int {
	...
	if errors.As(err, &exitErr) {
		...
		fmt.Fprintf(os.Stderr, "shevanio-ai-bench shim: %v\n", err)
		exitCode = 126
	}
