...
How Shevanio AI proves that an agent did what it was asked — in CI, on every push, with no API keys and no token cost.
...
| `shevanio-ai` binary | Yes | Compiled from the working tree, exposed as `SHEVANIO_AI_TEST_BINARY` |
...
Go test
 ...
 ├─ compile shevanio-ai                    → SHEVANIO_AI_TEST_BINARY
 ...
 └─ exec: opencode run --agent organic ...
       ...
       ▼
    [model fixture]  call #1 → "bash: shevanio-ai work capabilities"
       │
       ▼
...
SHEVANIO_AI_REAL_AGENT_E2E=1 \
  go test -v ./e2e/organicruntime \
  -run TestRealOpenCodeOrganicRuntimeJourneys -count=1 -timeout=15m
```

Without `SHEVANIO_AI_REAL_AGENT_E2E=1` the test skips, so ordinary `go test ./...` runs stay fast. A version mismatch on the `opencode` executable fails rather than silently testing a different runtime.
