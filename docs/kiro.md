...
This document explains how shevanio-ai integrates with **Kiro IDE** and what is installed in your local Kiro configuration.

## Overview

shevanio-ai supports Kiro as a **native-subagent** platform (`kiro-ide`).

When configured, shevanio-ai installs:

| Artifact | Path |
|----------|------|
| Steering file | `~/.kiro/steering/shevanio-ai.md` |
...
> **Auto-install not supported.** Kiro must be installed manually before running shevanio-ai.
...
shevanio-ai uses **two signals** to detect Kiro:
...
In practice: **the installer detects Kiro from `~/.kiro`**, not from `PATH`. If you have Kiro installed but `~/.kiro` hasn't been created yet (e.g., before first launch), run Kiro once to initialize its config dir, then re-run `shevanio-ai install`.
...
This follows the same SDD architecture used in shevanio-ai: orchestrator coordinates, phase agents execute, Engram persists artifacts across phases.
...
Kiro has a built-in spec workflow that shevanio-ai leverages. For medium and large changes, the orchestrator will use native Kiro artifacts at:
...
The steering file written by shevanio-ai uses the following frontmatter:
...
| Steering file | `%USERPROFILE%\.kiro\steering\shevanio-ai.md` |
...
Kiro uses a **split-root layout** — shevanio-ai managed files and IDE settings live in different directories:

- **Steering, skills, and native agents** → `~/.kiro/` (or `%USERPROFILE%\.kiro\` on Windows)
  - `~/.kiro/steering/shevanio-ai.md` — orchestrator persona
...
If shevanio-ai skills or steering are missing, check `~/.kiro/skills/` and `~/.kiro/steering/`.
