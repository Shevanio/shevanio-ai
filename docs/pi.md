...
Shevanio AI detects the `pi` binary first. If Pi is the only selected agent, the installer still provisions the real Engram component, but skips persona, ecosystem component selection, and Strict TDD prompts because `gentle-pi` owns those choices inside Pi.
...
The real Engram component is provisioned separately by Shevanio AI so `gentle-engram` has an Engram runtime to talk to.
...
`gentle-engram` owns the MCP schema itself. The installer runs `pi-engram init`, which initializes Pi's Engram MCP config under the Pi agent config directory instead of having Shevanio AI hand-write that file.

## Optional CodeGraph

Select CodeGraph during Shevanio AI installation to add its read-only MCP server to Pi. This integration is optional and owned entirely by Shevanio AI; `gentle-pi` is not modified.
...
| -------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| `/shevanio-ai:status`              | Shows package, SDD asset, OpenSpec, and model config status.                                                    |
| `/gentleman:persona`             | Switches between `gentleman` and `neutral` personas.                                                            |
| `/shevanio-ai:persona`             | Compatibility alias for `/gentleman:persona`.                                                                   |
| `/gentleman:models`              | Opens the Pi-native model assignment modal.                                                                     |
| `/shevanio-ai:models`              | Compatibility alias for `/gentleman:models`.                                                                    |
| `/sdd-init`                      | Bootstraps or refreshes `openspec/config.yaml`.                                                                 |
| `/shevanio-ai:install-sdd`         | Reinstalls SDD assets without overwriting local files.                                                          |
| `/shevanio-ai:install-sdd --force` | Force-refreshes installed SDD assets. Use this when you explicitly want package assets to replace local copies. |

## Persona Selection

Pi persona selection belongs to `gentle-pi`, not the Shevanio AI installer.
...
Pi model assignment belongs to `gentle-pi`, not the Shevanio AI installer.
...
.pi/shevanio-ai/support/strict-tdd.md
.pi/shevanio-ai/support/strict-tdd-verify.md
```

Use `/shevanio-ai:install-sdd --force` only when you want to replace local SDD assets with the package version.
...
| SDD agents are missing in Pi                           | Start Pi normally in the project so `gentle-pi` can run `session_start`, or run `/shevanio-ai:install-sdd`. If you used `pi -ns`, startup hooks were skipped.          |
...
| Memory tools or `/mcp` are missing                     | Re-run `shevanio-ai install --agent pi` to refresh `.pi/agent/settings.json`, `.pi/npm/package.json`, and the `pi-engram init` wiring, then check `/shevanio-ai:status`. |
