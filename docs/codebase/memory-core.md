...
Shevanio AI wires Engram into agents; Engram owns the memory store. This page explains the boundary so maintainers do not confuse installer code with memory database code.
...
Shevanio AI docs and prompt assets refer to these Engram concepts, but their schema is not defined in this repo.
...
AI agent receives prompt
  |
  v
Shevanio AI-installed prompt tells agent to use Engram MCP tools
  |
  v
Agent calls `engram mcp --tools=agent` via configured MCP entry
  ...
  |
  v
Engram runtime stores and searches memory outside shevanio-ai source
```
...
- [ ] Decide whether the change belongs to Shevanio AI wiring or Engram itself.
