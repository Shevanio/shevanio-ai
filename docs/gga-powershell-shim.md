...
When `shevanio-ai` installs GGA on Windows, it now installs a `gga.ps1` wrapper
...
check — re-running `shevanio-ai install` is idempotent.
...
`shevanio-ai install`**. In the final design this was moved to **runtime** — the `.ps1`
