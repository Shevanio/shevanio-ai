...
End-to-end tests that validate the `shevanio-ai` installer binary inside Docker containers running real Linux distributions.
...
docker build -f e2e/Dockerfile.ubuntu -t shevanio-ai-e2e-ubuntu .
docker run --rm shevanio-ai-e2e-ubuntu

# Run with full E2E on Arch
docker build -f e2e/Dockerfile.arch -t shevanio-ai-e2e-arch .
docker run --rm -e RUN_FULL_E2E=1 shevanio-ai-e2e-arch

# Interactive debugging
docker run --rm -it shevanio-ai-e2e-ubuntu /bin/bash
```
...
4. Build the `shevanio-ai` binary for `linux/amd64`
