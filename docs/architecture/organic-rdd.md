...
> Technical reference for PR [#1801](https://github.com/Shevanio/shevanio-ai/pull/1801). 154 commits, 340 files, +58,379 / −6,586. For the story behind it, see [the-organic-rdd-story.md](the-organic-rdd-story.md).
...
**The switch is a switch, and it starts off.** RDD is opt-in: until someone runs `shevanio-ai review mode enable --scope global`, RDD does not exist — nothing blocks, nothing gates, delivery falls to ordinary repository policy. `shevanio-ai review mode disable` returns to that same state. Turning it on re-validates from the current state rather than resuming stale obligations.
...
**`review capture-result`** admits one reviewer result per lens, bound to the frozen subject hash. `shevanio-ai review schema reviewer` emits the schema with a working example.
...
Two output modes, selected by the presence of `--contract shevanio-ai.review-integration/v1`:
...
```json
{
  ...
  "execute": {
    ...
    "command": "shevanio-ai review start --contract=... --target=sha256:... --projection=workspace",
    "arguments": [
...
**Git trust refusals** are typed rather than collapsed. shevanio-ai never provisions `safe.directory` and never relaxes an ownership check; the fix is diagnostic only, and detection requires three independent signals from one failure so a miss degrades to the generic message and can never mislabel.
...
`bench/` is a separate Go module that drives a real `shevanio-ai` binary through 36 end-to-end journeys and reports where the operator gets stuck. It is the evidence behind every friction claim in this branch, and it ships so the claims are reproducible rather than asserted.

```
cd bench && go run . run --binary $(command -v shevanio-ai)
```
