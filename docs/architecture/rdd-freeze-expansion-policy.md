...
2. Every blocking gate in this policy MUST offer a documented self-service exit that requires no source-code archaeology. Reference pattern: `skills/shevanio-ai-chained-pr`'s `size:exception` label lets a maintainer clear the 400-line budget gate without reading the budget-check implementation. This policy's exits are named explicitly in **Escalation path** below — never "read the source to find out."
