...
```bash
go run ./cmd/shevanio-ai install [flags]
```
...
|----------|--------|-------------|
| `SHEVANIO_AI_INSTALL_SCOPE` | `global` \| `workspace` | Sets the install scope without a flag. Useful in CI. Equivalent to `--scope`. Default: `global`. |
