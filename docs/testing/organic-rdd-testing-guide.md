...
> [!WARNING]
> **Historical and superseded guide.** This document preserves the candidate-specific validation procedure for `v2.2.0-rc.1` and PR [#1801](https://github.com/Shevanio/shevanio-ai/pull/1801). It is not current installation or validation guidance for stable [`v2.5.0`](https://github.com/Shevanio/shevanio-ai/releases/tag/v2.5.0) or unreleased `main`. Use the [README installation guide](../../README.md#quick-start) for current installation channels and validation entry points.
>
> Community testing guide for the candidate built from PR [#1801](https://github.com/Shevanio/shevanio-ai/pull/1801). Every **Expected** here was validated against real output before publication. The guide uses a throwaway HOME precisely so it does not touch your real config — do not skip the setup.
...
>
> ```
> shevanio-ai --version          # or "$RC_BIN" --version
> ```
>
...
The binaries are on the prerelease page: **https://github.com/Shevanio/shevanio-ai/releases/tag/v2.2.0-rc.1**
...
2. Verify the checksum against `SHA256SUMS.txt`:
   ...
   ```
3. Make it runnable and confirm which build you have. **You do not need shevanio-ai installed already** — this works on a clean machine:
   ```
   RC_BIN="$(pwd)/shevanio-ai_2.2.0-rc.1_<os>_<arch>"
   chmod +x "$RC_BIN"
   "$RC_BIN" --version
   ```
4. Either invoke `"$RC_BIN"` explicitly for every step, or put it on your PATH under a throwaway directory:
   ```
   mkdir -p /tmp/rdd-bin && ln -sf "$RC_BIN" /tmp/rdd-bin/shevanio-ai
   export PATH="/tmp/rdd-bin:$PATH"
   ```
5. **Only if you already had shevanio-ai installed** and want to replace it, back the old one up first:
   ```
   command -v shevanio-ai && cp "$(command -v shevanio-ai)" ~/shevanio-ai.backup
   ```
   Roll back with `mv ~/shevanio-ai.backup "$(command -v shevanio-ai)"` when you are done.
...
1. [ ] `shevanio-ai install --scope workspace --agents claude-code --components permissions` → **Expected**: it installs and ends with "You're ready", without asking anything about SDD.
...
4. [ ] Search for `review mode` → **Expected**: `shevanio-ai review mode enable|disable|status` shows up.
5. [ ] Run the same install again → **Expected**: same output and the files do NOT change.

### Flow 2: Kill switch

1. [ ] `shevanio-ai review mode status --cwd $HOME/demo --json` → **Expected**: effective `off`, source `default` — receipt-driven development is opt-in, so a fresh install reviews nothing until you ask it to.
2. [ ] `shevanio-ai review start --cwd $HOME/demo` → **Expected**: refused, naming that reviews are off **and naming the command that turns them on**:

```
receipt-driven development is disabled: start is rejected because the default mode source
keeps it off; turn reviews on with shevanio-ai review mode enable --scope=global
```

It does NOT hang, it does NOT review. A refusal that exits non-zero and names no command is the defect.
3. [ ] `shevanio-ai review mode enable --scope global --cwd $HOME/demo` then `status` → **Expected**: effective `on`, source `global`.
4. [ ] `shevanio-ai review mode disable --cwd $HOME/demo` → **Expected**: it confirms reviews are off.
5. [ ] `status` again → **Expected**: effective `off`, source `global` (an explicit off, not the default).
6. [ ] `shevanio-ai review start --cwd $HOME/demo` → **Expected**: the same shape of refusal, naming commands that actually reach `on` from where you are. If you turned it off at clone scope, the message must name `--scope=global` **then** `--scope=clone`: clearing the clone override alone drops you on the opt-in default, still off.
...
2. [ ] `shevanio-ai review start --cwd $HOME/demo` → **Expected**: `risk_level: low`, `selected_lenses: []` — zero reviewers, no question.
...
1. [ ] With a tier 1/2 change ready, `review start` in an interactive terminal → **Expected**: **two** options — `1) Run the review now` / `2) Not now, just this once` — and a final line naming `shevanio-ai review mode disable`. **There is no option 3.**
...
2. [ ] `shevanio-ai review validate --gate pre-push --cwd $HOME/demo` → **Expected**: `"delivery": "disabled/unmanaged"`, `"allowed": false`, **exit 0**. It reports, it does not block.
...
```
review lifecycle gate denied: scope-changed: recover via shevanio-ai review recover
...
4. [ ] `shevanio-ai review start --cwd "$PWD"` → **Expected**: the review **starts**. It used to blow up with `Needed a single revision`.
...
```
shevanio-ai review status --next-transition --contract shevanio-ai.review-integration/v1
```
...
3. [ ] Read the `next_transition.execute.command` field → **Expected**: one complete line, starting with `shevanio-ai review <verb>`, carrying every argument from step 2 in the same order and in `--flag=value` form. You never assemble it yourself: `operation` is a logical name (`review.start`), `command` is the runnable line.
...
6. [ ] Paste those collect tokens straight into `shevanio-ai review capture-result` → **Expected**: it runs. **Do not add `--cwd`**: the tokens already carry `--repository-context`, and passing both is refused. If you hit that refusal, report whether it told you which one to drop.
...
2. [ ] With that review in `validating` and no captured evidence, run `shevanio-ai review finalize --lineage <id> --cwd .` → **Expected**: an error that **names both commands** to get out:

```
finalize for lineage "<id>" had no verification evidence to consume and made no
transition; capture it first with `shevanio-ai review capture-evidence`, then run
`shevanio-ai review finalize --lineage <id> --captured-evidence`
```
...
4. [ ] Turn reviews on and run `review status --contract shevanio-ai.review-integration/v1` → **Expected**: it says nothing governs the candidate and that you should start a new one. The old lineages show up listed as a recovery option, **not** as a list you are forced to pick from.
...
1. [ ] With a review in progress waiting for results, run `review finalize --lineage <id>` → **Expected**: the error names `shevanio-ai review capture-result`.
2. [ ] Run a gate in a repo **with no review at all**, asking for the negotiated envelope:

```
shevanio-ai review validate --gate pre-commit --contract shevanio-ai.review-integration/v1
```
...
```
$EDITOR "$(git rev-parse --git-common-dir)"/shevanio-ai/review-transactions/v2/<lineage>/review-receipt.json
```
...
```
... A defect report was saved at <...>/shevanio-ai/defect-reports/receipt-publication-conflict-<hash>.md
-- file it at https://github.com/Shevanio/shevanio-ai/issues/new/choose.
```
...
git add guide.md
shevanio-ai review start --cwd .
shevanio-ai review finalize --cwd .
shevanio-ai review validate --gate pre-commit --cwd .
```
...
```
shevanio-ai review status --next-transition --contract shevanio-ai.review-integration/v1 --cwd . > /tmp/rdd-out/nt.json
```
...
1. [ ] On Windows with Go 1.25.10+ on PATH, run a command that triggers the update check with an older shevanio-ai installed → **Expected**: it upgrades itself. It does **not** print "requires manual update", and it does **not** send you to a releases page.
2. [ ] `shevanio-ai --version` afterwards → **Expected**: the new version.
...
4. [ ] If your `shevanio-ai` is a symlink into the go-install directory → **Expected**: treated as a match, no warning. A spurious warning there is also a defect.
...
2. [ ] `shevanio-ai review status --cwd .` → **Expected**: it works. Status is
   read-only by contract and must not need to write anything, not even a lock
   file.
3. [ ] `shevanio-ai review start --cwd .` → **Expected**: a typed refusal naming
...
`shevanio-ai/review-transactions/v2/<lineage>/`, and the lineage identifier is
...
```powershell
$p = "$(git rev-parse --git-common-dir)\shevanio-ai\review-transactions\v2"
$p.Length
```
...
```
shevanio-ai review start --cwd . && shevanio-ai review finalize --cwd .
cat "$(git rev-parse --git-common-dir)"/shevanio-ai/review-transactions/v2/<lineage>/review-receipt.json
```
...
3. [ ] `shevanio-ai review validate --gate pre-commit --cwd .` → **Expected**:
   `allow`, unchanged. The approval is bound to bytes that did not move.
4. [ ] `shevanio-ai review status --cwd .` → **Expected**: the same lineage and
...
5. [ ] The kill switch keeps a timestamp that the design says is provenance
   only. Test that directly: `shevanio-ai review mode disable`, then
   `shevanio-ai review mode enable`, then move the clock back past
   `rdd_mode_recorded_at` in `$HOME/.shevanio-ai/state.json`, then
   `shevanio-ai review mode status --json` → **Expected**: `effective: on`, and
...
`shevanio-ai sdd-status <change> --json` reported the archive dependency `ready`
...
1. [ ] `shevanio-ai install` (or `shevanio-ai sync`) into a throwaway HOME, then
...
2. [ ] In a repository with a complete, verified SDD change, run
   `shevanio-ai review mode disable` and then
   `shevanio-ai sdd-status <change> --json` → **Historical candidate expectation,
...
**Never write command output inside the repository under test.** The review target is derived from the workspace snapshot, so `shevanio-ai ... > out.txt` run from inside the repo adds an untracked file and changes the very thing being measured. A transition proposed before the redirect no longer matches after it, and you get a refusal that has nothing to do with what you were testing. Keep a separate directory:

```
mkdir -p /tmp/rdd-out
cd $HOME/demo
shevanio-ai review start --cwd . > /tmp/rdd-out/o.txt 2> /tmp/rdd-out/e.txt
```
...
This section is retained to explain the original candidate procedure. Do not open a current issue or comment on PR [#1801](https://github.com/Shevanio/shevanio-ai/pull/1801) for results from `v2.2.0-rc.1`; that candidate process is complete. For a current concern, first reproduce it against stable [`v2.5.0`](https://github.com/Shevanio/shevanio-ai/releases/tag/v2.5.0), then use the repository's current contribution and issue workflow.
