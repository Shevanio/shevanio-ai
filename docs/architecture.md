...
```
cmd/shevanio-ai/             CLI entrypoint
internal/
...
shevanio-ai install --dry-run --agent claude-code --preset minimal

# Dry-run smoke test (Windows PowerShell)
shevanio-ai.exe install --dry-run --agent claude-code --preset minimal
```
...
Install Gentleman.Dots first for your dev environment, then Shevanio AI for the AI layer on top.
