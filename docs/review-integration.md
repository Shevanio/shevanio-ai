...
Shevanio AI exposes two negotiated review contracts. `shevanio-ai.review-integration/v1` preserves the published Base64 candidate-diff transport byte for byte. `shevanio-ai.review-integration/v2` is the native-Git contract: it carries immutable base/candidate tree IDs and an ordered changed-path manifest, never an inline patch. Its current minor is v2.2: v2.1 selected the immutable review runtime explicitly, and v2.2 adds provider submission descriptors for the collect-satisfying capture operations. Both let a consumer reconstruct one target after restart, drive explicit review operations, and validate the resulting receipt without reading provider-private authority files.

## Advisory Model Review

Go owns the reviewer provider contract for Claude Code, OpenCode, Codex, and Pi. OpenCode relays one ordinary host Task through one live `shevanio-ai review opencode-transport` process; Go materializes the prompt, validates the completion, admits raw output, and captures authority. Only after immutable slot capture and readback does Go publish the `provider_contract` descriptor and emit the result frame. The descriptor means Go-materialized and Go-admitted output reached the live provider transport; it is not UI provenance. The adapter never parses bindings, schemas, or reviewer output.
...
OpenCode provider review runs from the normal session. It does not require a restart, a special terminal, or `OPENCODE_DISABLE_*` variables. Runtime adapters only transport opaque frames; Go owns evidence bounds, schema, validation, admission, and capture.

## Negotiate the provider first

Resolve the exact `shevanio-ai` executable that will perform review operations, then query it outside a repository:

```bash
shevanio-ai review capabilities \
...
New integrations SHOULD negotiate `shevanio-ai.review-integration/v2`. The current response is capabilities v2.2 (status v5, consent v3) and requires an advertised runtime identity through `--agent` for negotiated STATUS and START. Capabilities v2.0 and v2.1, status v3 and v4, and consent v2 remain historical byte-pinned artifacts; they are not rewritten or emitted as the current response. Existing v1 consumers remain valid and MUST continue validating the published v1 schemas and fixture bytes; they do not gain tree-only fields additively.

Protocol v1.5 advertises `shevanio-ai.review-integration.capabilities/v1.5` and adds `outcome_bound_verification_evidence` without changing the preserved v1.4 identity or its `one_shot_final_verification_retry` feature. `review capture-evidence` requires one closed `--outcome` (`passed`, `verification_failed`, or `procedural_tooling_failed`) and persists `shevanio-ai.review-verification-evidence/v2` beside immutable candidate-addressed raw bytes. The record binds lineage, authority revision, target tree, canonical paths and ledger IDs, raw SHA-256 and size, outcome, and its own canonical digest. FINALIZE derives captured approval or escalation from that record; a caller-supplied `--failed` may agree with it but cannot override it.

Protocol v1.4 advertises `shevanio-ai.review-integration.capabilities/v1.4` and adds `one_shot_final_verification_retry`, operation `review.retry_final_verification`, and incident schema `shevanio-ai.review-final-verification-incident/v1`. This is a dedicated provider-owned retry for one exact completed failed final-verification tooling incident; it does not relax generic recovery.

Protocol v1.3 introduced `provider_artifact_admission`, `validating_result_reopen`, `recovered_correction_evidence`, and `classified_authority_repair`. START v2 supplies one provider-owned `ArtifactSubject` per selected lens. Result artifact v2 and status v2 expose the admitted subject hash and completed admission decision. Status v2 also requires a bounded `shevanio-ai.review-authority-repair-assessment/v1`; `review.repair` publishes the matching strict preflight and execution contract. The durable admitted-result envelope preserves raw and canonical payload identities, the result identity, and repository-verified candidate-causal finding IDs. Exact accounting-only recovery may reuse that evidence only when the corrected predecessor bytes are the successor's exact initial target. Protocol v1.0 through v1.4 capability schemas and fixtures remain packaged unchanged. Consumers must reject an unknown schema/minor identity they do not support; v1.5 consumers validate the v1.5 schema before relying on the current features.
...
```bash
shevanio-ai review start --contract shevanio-ai.review-integration/v2 --agent claude-code --cwd .
shevanio-ai review status --contract shevanio-ai.review-integration/v2 --agent claude-code --cwd .
shevanio-ai review finalize --contract shevanio-ai.review-integration/v2 --cwd . --lineage <lineage> ...
shevanio-ai review validate --contract shevanio-ai.review-integration/v2 --cwd . --gate pre-commit
shevanio-ai review bind-sdd --contract shevanio-ai.review-integration/v2 --cwd . --change <change> --lineage <lineage> --expected-binding-revision=<revision>
```
...
Consumers MUST NOT reconstruct receipts, derive canonical hashes, inspect the Git common-dir authority store, select an ambiguous lineage automatically, or infer that a transport interruption did not mutate state. Shevanio AI does not choose models, run arbitrary user commands, or replace a consumer's command-safety policy.
...
Claude Code, OpenCode, Codex, and Pi provide the enforced reviewer-execution boundary through one shared provider contract: Claude reviewers are tool-free fresh agents with prompt-carried native evidence; OpenCode relays a task through its ordinary running session with no restart, isolation session, network listener, temporary candidate bytes, child OpenCode process, or `OPENCODE_DISABLE_*` variables; Codex launches its provider-bound subprocess; and Pi uses a `gentle-pi`-owned host relay that launches a fresh locked-down print-mode `pi` subprocess in an empty scratch directory, forwards the Go-issued opaque prompt untouched, and returns the raw final bytes through the exact capture operation. The Pi relay must declare the exact `gentle-pi.review-relay/v1` handshake; without it, `review start --agent pi` refuses before any repository, target, or authority work. Kilo receives `immutable_review_transport_unsupported` before repository, target, authority, collection, or process work; that typed result names the clone-scoped review-mode exit and the supported runtimes. Normal SDD and the manual compatibility path remain available; no provider or model is substituted.
...
```powershell
$manifest = & shevanio-ai review capture-result --repository-context $repositoryContext --expected-revision $revision --lineage $lineage --target $target --lens $lens --order $order --input $resultPath
$manifestPath = Join-Path $env:TEMP "shevanio-ai-review-manifest.json"
...
This is a historical compatibility boundary introduced in `v2.2.0`, not the current release version. Work delivered while the kill switch is off and no exact governing receipt applies is recorded as unmanaged, and it stays recorded as unmanaged: native lifecycle gates report `disabled/unmanaged` at exit 0, the change closes under ordinary repository policy, and nothing — not a stale receipt, not a later empty-candidate approval — may ever make that window read as reviewed. SDD status differs: it structurally omits `reviewGate` while the kill switch is off, and also when it is on but no review was discovered for the candidate; archive proceeds under ordinary policy in both cases. A present `reviewGate.result: allow` is required only for discovered review activity. Re-enabling re-validates the current state through one full fresh review, exactly as if receipt-driven development had never run: every downstream stop over unreviewed content names `shevanio-ai review start` (with `--base-ref <commit>` when the delivered work is already committed), and completing that review is what unblocks the stop. The fresh review subsumes the unmanaged history; durable retroactive reconciliation — per-delivery dispositions, retroactive receipts, or any ledger that blesses past unmanaged deliveries — is deliberately not part of that historical release boundary.
...
Run `shevanio-ai review repair --preflight --cwd <repo>` before collecting maintainer intent. The path-free response supplies only provider-owned class, lineage, expected revision, cause, disposition, opaque repository binding, and authorization schema. The maintainer then provides `actor`, `reason`, and the exact nine-line LF-only authorization. An authorized STATUS transition marks `maintainer-authorization` as `provided` instead of echoing the completed authorization; the controller must reuse the exact secret it already holds. No response emits a repository path, quarantine record, or ready-made authorization string. The direct `repair-legacy-alias` verb remains compatibility-only.
...
Run `shevanio-ai review abandon --cwd <repo> --lineage <lineage> --expected-revision <revision> --reason operator_disposition --actor <actor> --maintainer-authorization <binding>`.
...
A captured targeted validation whose evidence reports the immutable candidate could not be inspected carries no verdict in either direction, so while its correction still has recapture attempts left it is neither admitted nor treated as unverifiable evidence. Routing answers `targeted_validation_inconclusive_recapture_required` in exactly the shape `targeted_validation_required` would take for the same runtime: a host-mediated runtime receives its Go-issued provider role task, and a caller-authored runtime receives the `external.run_targeted_validation` input with its FINALIZE submission descriptor. A host-mediated runtime is never handed the caller-authored form, because only Go may run its validator. Nothing transitioned and the correction attempt was not consumed; restore the validator's read-only access to the frozen trees and run the same targeted validation again. The caller-authored submission goes straight to FINALIZE and never touches the occupied slot. A host-mediated recapture publishes into that slot, so its capture first archives the non-verdict occupant under the store's quarantine root and vacates the slot: the exact superseded bytes and digest are preserved, and an occupant that is an admitted verdict -- passed or failed -- is never moved and never replaced. A captured validator result that is corrupt, partially published, non-canonical, or bound to another request remains `captured_artifacts_unverifiable`.
The recapture is bounded, not unlimited. Every recapture that is itself a non-verdict is rejected and recorded under the same quarantine root as one numbered attempt, and that per-correction ledger is the bound: the occupant counts as the first non-verdict and one correction admits three in total. The ledger counts rejections rather than the slot, because this build refuses to publish a non-verdict at all, so a repeatedly blind validator changes nothing else; identical bytes across attempts are the expected shape of that failure and are never collapsed into one attempt. Once the third is recorded, routing stops offering a recapture and answers `captured_artifacts_unverifiable`. That code is reused rather than minted, and it fits only partly: the stored bytes are provably intact here and the fault is tree access, so its "inspect the review authority store" continuation is the weaker half. The operative exits are the two it shares with every terminal stop -- change the corrected candidate, which derives a new correction target with its own empty ledger and a fresh validation, or run `shevanio-ai review mode disable --scope clone --cwd <repo>` to deliver under ordinary repository policy. A dedicated reason code for an exhausted validation budget is the right long-term answer and is tracked separately.

Do not answer either form with a bare `shevanio-ai review finalize --lineage <id>`. That call carries no validation and no captured-evidence binding, so FINALIZE keeps the original frozen review target as its candidate and refuses with `stale_target_identity` when the live workspace holds the corrected content. Execute the transition STATUS emitted.

On the host-mediated runtimes the validator never sees that transition input, so the same recipe travels inside the Go-issued provider request: it carries `repository_context` alongside `validation_request`, and the role prompt names the exact `review inspect-candidate --purpose targeted-validation` form built from those two fields. A validator that cannot run commands still holds the complete frozen patch for every corrected path in `evidence`, which is authoritative immutable-tree content rather than a summary. Reporting that the candidate could not be inspected is a last resort on every runtime, and an inconclusive inspection is never recorded as a failed check: a failed check spends the lineage's single correction attempt.
...
| `corrected_candidate_unavailable` | Two distinct situations share this code; pick the one that is true. When the review found real defects: change the candidate content so it differs from the frozen original, then re-run `shevanio-ai review status --cwd <repo> --contract shevanio-ai.review-integration/v2 --agent claude-code --next-transition` (or `shevanio-ai review finalize --lineage <id>`) to receive the `targeted_validation` collection — see "After a correction forecast and an actual candidate change" above. When the reviewers were given the wrong input and their findings describe content that was never the candidate: a maintainer quarantines those admitted results and reopens their lenses over the same frozen candidate with `shevanio-ai review reopen-results --prepare --cwd <repo> --lineage <id> --expected-revision <revision> --target <target> --reason <reason> --actor <actor> --quarantine-lens <lens>` (repeat per affected lens), then applies the emitted authorization; the overridden result bytes are preserved in quarantine and named in the audit record. |
| `empty_base_diff_bootstrap_required` | Terminal for this zero-path selection — STATUS must not re-collect the already-supplied base or emit a START that preflight will refuse. If this is the authorized empty-root first-publication bootstrap from #1641, a maintainer must insert an empty root below the content commit; then run `shevanio-ai review status --cwd <repo> --contract shevanio-ai.review-integration/v2 --agent claude-code --next-transition --base-ref <empty-root> --committed-only`. |
| `lens_context_budget_exceeded` | Terminal for this frozen candidate — complete immutable reviewer evidence exceeds the native budget and is never truncated. Reduce the candidate scope or target identity, then run `shevanio-ai review start` for the new candidate. Alternatively, run `shevanio-ai review mode disable --scope clone --cwd <repo>` to exit receipt-driven review under ordinary repository policy; do not change the runtime, provider, or toolchain. |
| `correction_repository_verification_failed` | Change the correction candidate within the same open frozen budget, then re-run `shevanio-ai review status --cwd <repo> --contract shevanio-ai.review-integration/v2 --agent claude-code --next-transition`. The failed candidate's evidence remains immutable under its own identity; no correction attempt or changed-line accounting was consumed. |
| `corrupted_or_unverifiable_authority` | Terminal — `shevanio-ai review repair --preflight --cwd <repo>` classified this authority as `unsupported`, `ambiguous`, `conflicting`, or `truncated` rather than `eligible`. Requires a maintainer to inspect the review authority store directly; automated repair cannot proceed. |
...
| `original_finalize_request_required` | Re-run `shevanio-ai review finalize --lineage <id>` with the exact original content-bound payload (results/evidence). A different payload is a typed reconciliation failure, not a retry — see "Re-run a non-terminal FINALIZE" above. |
...
| `rdd_disabled` | Run the exact source-scoped `shevanio-ai review mode enable` command rendered with this STATUS result, then re-run its exact repository-bound STATUS command. Receipt-driven development is opt-in, so the most common case is a repository where nobody enabled it: the deciding source is `default` and the rendered command is `--scope=global`, the only scope that can turn review on. |
| `staged_delivery_candidate_required` | Stage every reviewed path exactly as it was reviewed, then re-run `shevanio-ai review status --cwd <repo> --contract shevanio-ai.review-integration/v2 --agent claude-code --lineage <id> --projection staged --gate pre-commit --next-transition`. STATUS returns `review.validate` only when that staged candidate exactly matches the approved receipt. |
| `staged_workspace_overlay_recovery_unavailable` | Terminal for a fresh target — staged projection combined with `--workspace-overlay` is recovery-only. Pass `--lineage <id>` to recover an existing lineage, or drop `--workspace-overlay` and run `shevanio-ai review start --projection staged` to start fresh. |
| `unchanged_or_unverified_authority` | Terminal — the single correction attempt for this lineage is already consumed without a verified candidate change. `shevanio-ai review start` on this exact unchanged candidate only resumes this same lineage, not a fresh one. Change the candidate content first, then run `shevanio-ai review start` to begin a genuinely new lineage. |

### Retry one failed final verification

`shevanio-ai review retry-final-verification` is provider-only and one-shot across the entire ancestry. Eligibility requires one exact compact-v2 leaf at the supplied revision in `escalated`, its matching receipt, exactly one completed receipt-published FINALIZE attempt whose last transition is `review/complete-verification`, a `procedural_tooling_failed` record whose digest and raw bytes match both journal and terminal bindings, and an unchanged exact live `CurrentSnapshot`. A genuine `verification_failed` outcome is permanently ineligible. Reviewer-result, correction, scoped-validator, SDD, ambiguous, superseded, already-retried, or target-drift states are also ineligible.

The incident file must be the compact canonical JSON object plus one LF for `shevanio-ai.review-final-verification-incident/v1`. Its only class is `procedural_tooling_failure`; it binds predecessor lineage, terminal and validating revisions, current target identity, failed-evidence hash, and FINALIZE request digest. Inspect its closed shape with `shevanio-ai review schema final-verification-incident`; the native parser additionally enforces canonical bytes.

The maintainer authorization is exact LF-only text in this order: `shevanio-ai.review-final-verification-retry-authorization/v1`, predecessor lineage and revision, successor lineage, validating revision, target identity, failed-evidence hash, FINALIZE request digest, incident class and digest, actor, and reason. Public STATUS emits only path-free provider inputs and collects this authorization externally; it never emits the completed authorization or the failed-evidence path.
...
Three gate denials are truthful availability classifications, never corruption verdicts. `remote_fetch_required` means every unknown receipt assessment failed only because the advertised publication tip is not in the local object store: the authority store is healthy, the gate stays denied fail-closed but retry-safe (`next_action: retry`), and the denial names the exact `git fetch <remote>` continuation — running it and re-running the identical gate is the complete recovery. `inventory_busy` names transient shared-store coordination — the compact store lock held by a live concurrent review operation, a bounded lock-wait loss, or readable interrupted-holder residue — including the recorded holder; it is retry-safe because the condition clears itself. `lock_holder_unverifiable` names store-lock residue whose recorded holder is neither flock-held nor verifiably dead on this host; no event terminates a retry, so it is not retry-safe. A receipt that exists but fails integrity validation remains `authority_corrupted` even while the shared lock is verifiably held; none of these codes weaken that verdict.

### Preserve the uniform failure envelope

Every failed negotiated operation emits the failure envelope matching its contract: `failure/v1` for integration v1 and `failure/v2` for integration v2, and still exits nonzero. Capabilities defaults to v1; repository operations use the selected envelope when `--contract` is present. The collect-satisfying capture operations (`review capture-result`, `review capture-evidence`, `review capture-refuter`, `review capture-validation`) accept no `--contract` yet also emit `failure/v2` on stdout for every refusal, unconditionally: their success paths already print JSON on stdout, and a machine caller receiving bare stderr with empty stdout could only classify a refusal that provably never started as an unknown mutation outcome. Their operation names (`review.capture-result`, `review.capture-evidence`, `review.capture-refuter`, `review.capture-validation`) exist only in the v2 failure schema's `operation` enum, never in v1's and never in the capabilities `operations` array. A binding refusal is typed: `verification_evidence_binding_mismatch`, `capture_binding_mismatch`, `reviewer_result_slot_occupied`, and `captured_final_evidence_conflict` all report `mutation_outcome: not_started` with `next_action: review.status`, because a fresh STATUS re-renders the exact slot tokens. The operator-facing stderr line keeps the native refusal text, now suffixed with the machine code. Other unnegotiated command errors retain their compatibility behavior.
...
Terminal compact receipts are published with a synced temporary file and a platform-native atomic no-clobber operation: an exact existing byte sequence is an idempotent success, while different or non-regular existing content is rejected without replacement. On filesystems that support directory synchronization, the parent directory is synced after publication. Windows may reject directory-handle synchronization; Shevanio AI still provides atomic visibility and conflict rejection there, but does not claim power-loss durability for the directory entry.
...
1. Resolve and independently verify the exact Shevanio AI executable.
...
Pi adoption, fallback retirement, package pinning, and Pi release sequencing are separate consumer work. They do not change Shevanio AI's provider authority or release ownership.
...
- `contracts/review-integration/v2/schemas/` — 17 strict JSON Schemas, including current capabilities v2.2, status v5, consent v3, and the capture-result dry-run response contract.
